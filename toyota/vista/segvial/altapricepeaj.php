	           ...
	           <fieldset class="ui-widget ui-widget-content ui-corner-all">
                         ...
                         <tr>
                             ...
                             <?
                               ...
                               $i = 0;
                               print "<td align='center' colspan=".mysql_num_rows($result).">Tipos Unidades</td></tr><tr><td></td>";
                               ...
                               print "</tr>";
                               $sql = "SELECT id, concat(id_estacionpeaje,id_tipounidad) as clave, LPAD(round(precio_peaje,2),5,'0') as precio_peaje FROM preciopeajeunidad p where (id_estructura = ".STRUCTURED.")";
                               $result = mysql_query($sql);
                               $importes = array();
                               while ($data = mysql_fetch_array($result)){
                                     $importes[$data['clave']] = $data['precio_peaje'].'-'.$data['id'];
                               }
                               ...
                               while ($data = mysql_fetch_array($result)){
                                     ...
                                     for($i=0; $i < count($array); $i++){
                                               $key = $data['id'];
                                               $key.=$array[$i];
                                               $imp = explode('-', $importes[$key]);
                                               print "<td><input name='$data[id]-$array[$i]-$imp[1]' type='text' size='5' value='$imp[0]'></td>";
                                     }
                                     print "</tr>";
                               }
                             ...
                             ?>
                             <tr><td align="right" colspan="<?echo (count($array) + 1);?>"><input type="button" id="save" value="Guardar Datos"></td></tr>
                         </table>
            </fieldset>
