...
cast(if (o.hllegadaplantareal <= o.hllegada,1,
  ...
  cast(if (o.hsalidaplantareal <= o.hsalida,1,
      ...
      FROM (select o.id, id_estructura_servicio, hsalidaplantareal, hllegadaplantareal, id_ciudad_origen, id_estructura_ciudad_origen,
            ...
            where month(o.fservicio) between 2 and $_GET[mes] and year(o.fservicio) = $_GET[anio] and o.id_estructura = $_GET[str] and o.id_cliente = $_GET[cli] and not o.borrada and not o.suspendida and (hllegadaplantareal is not null || hsalidaplantareal is not null)) o
         ...
         //   $pdf->Cell(20,7, number_format($efic,2),0, 0, 'R', false);
            ...
            foreach ($mese_compara as $key => $value){
                    ...
                    foreach ($value[0] as $clave => $valor) {       ///imprime la cant de servicios
                            ...
                            $pdf->SetXY($posx+70+$despx, $auy+$aux_claves[$clave]);
...
from (
    ...
    from (
          ...
          left join (select * from servicios where id_estructura = $_GET[str]) s on s.id = o.id_servicio
          left join tipotnoordenes tto on tto.id_orden = o.id
          where month(o.fservicio) = $_GET[mes] and year(o.fservicio) = $_GET[anio] and o.id_estructura = $_GET[str] and o.id_cliente = $_GET[cli] and not o.borrada and not o.suspendida
          ) o
...
from (
    ...
    from (
          select o.id, id_estructura_servicio, fservicio, cantpax, id_servicio, o.hllegada, o.hsalida, km, id_micro,
                 ...
                 if (tto.id_tipo_servicio is null, s.id_TipoServicio, tto.id_tipo_servicio) as id_tipo_servicio,
                 if (tto.i_v is null, s.i_v, tto.i_v) as i_v,
                 concat(date_format(fservicio, '%m'),'-',year(fservicio)) as periodo
          from ordenes o
          left join (select * from servicios where id_estructura = $_GET[str]) s on s.id = o.id_servicio
          left join tipotnoordenes tto on tto.id_orden = o.id
          where month(o.fservicio) between 2 and $_GET[mes] and year(o.fservicio) = 2016 and o.id_estructura =  $_GET[str] and o.id_cliente = $_GET[cli] and not o.borrada and not o.suspendida
          ) o
    left JOIN (Select * from tiposervicio where (id_estructura = $_GET[str]) and (id not in (16, 3))) ts ON ts.id = id_tipo_servicio
    left JOIN turnos tu ON tu.id = id_turno
    where i_v = 'i'
          ...
          //  $pdf->ln();recuperr esta linea
            ...
            $sql="SELECT periodo, round(sum(anio)/count(*)) as media
                  from ( select id_micro, concat(date_format(fservicio,'%m'),'-',year(fservicio)) as periodo
                         from ordenes o
                          where month(o.fservicio) between 2 and $_GET[mes] and year(o.fservicio) = $_GET[anio] and o.id_estructura = $_GET[str] and o.id_cliente = $_GET[cli] and not o.borrada and not o.suspendida) o
