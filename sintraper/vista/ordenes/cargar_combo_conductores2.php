<?
  ...
  $orden = $_POST['orden'];
           ...
           $sql = "SELECT *
                   ...
                   where ((select fservicio from ordenes where id = $orden) between desde and hasta) and (em.id_estructura = $estructura) and (e.activo) and (n.id_estructura = $estructura) and (id_cargo = 1) and (n.activa)
                   )
                   union
                   ...
                   where (e.activo) and (em.id_estructura = $estructura) and (e.id_estructura = $estructura) and (id_cargo = 1) and (e.id_empleado not in (SELECT e.id_empleado
                                                                                                            ...
                                                                                                            where ((select fservicio from ordenes where id = $orden)  between desde and hasta) and (e.activo) and (nov.activa) and (nov.id_estructura = $estructura) and (id_cargo = 1)
