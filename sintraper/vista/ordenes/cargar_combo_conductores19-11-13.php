<?
  ...
  $orden = $_POST['orden'];
           ...
           $sql = "SELECT *
                   ...
                   where ((select fservicio from ordenes where id = $orden) between desde and hasta) and (e.id_estructura = $estructura) and (e.activo) and (not borrado) and (id_cargo = 1) and (n.activa)
                   )
                   union
                   ...
                   from empleados e
                   inner join empleadores em on em.id = e.id_empleador
                   where (e.activo)and (not borrado) and (e.id_estructura = $estructura) and (id_cargo = 1) and (e.id_empleado not in (SELECT e.id_empleado
                                                                                                            from novedades nov
                                                                                                            inner join empleados e on e.id_empleado = nov.id_empleado
                                                                                                            where ((select fservicio from ordenes where id = $orden)  between desde and hasta) and (e.activo) and (nov.activa) and (id_cargo = 1)
           ...
           $sql = "SELECT *
                   ...
                   inner join cod_novedades cn on cn.id = n.id_novedad
                   inner join empleadores em on em.id = e.id_empleador
                   where ('$fecha' between desde and hasta) and (em.id_estructura = $estructura) and (e.activo) and (n.id_estructura = $estructura) and (id_cargo = 1) and (n.activa)
                   )
                   union
                   (select e.id_empleado, 'CONDUCTORES DISPONIBLES' as nov, if(em.id = 1,upper(concat(e.apellido, ', ',e.nombre)), upper(concat('(',em.razon_social,') ', e.apellido, ', ',e.nombre))) as emple, 0 as id_nov
                   from empleados e
                   inner join empleadores em on em.id = e.id_empleador
                   where (e.activo) and (em.id_estructura = $estructura) and (e.id_estructura = $estructura) and (id_cargo = 1) and (e.id_empleado not in (SELECT e.id_empleado
                                                                                                            from novedades nov
                                                                                                            inner join empleados e on e.id_empleado = nov.id_empleado
                                                                                                            where ('$fecha'  between desde and hasta) and (e.activo) and (nov.activa) and (nov.id_estructura = $estructura) and (id_cargo = 1)
                                                                                                            group by e.id_empleado))
                   )) e
                   order by id_nov, emple";
           }
