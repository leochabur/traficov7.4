<?php
  ...
  if($accion == 'load'){
          $conn = conexcion();
          $sql = "select descr, id_empleado, haber, debe, round(@imonto:= @imonto + (haber - debe),0) AS Saldos
from( SELECT 'Vacaciones Inical' as descr, id_empleado, cant_dias as haber, 0 as debe, '2014-06-30' as fecha_liquidacion
      ...
      where id_empleado = $_POST[emple] and anio <> 0
      union all
      SELECT concat('Vacaciones (', date_format(desde, '%d/%m/%Y'),' - ',date_format(hasta, '%d/%m/%Y'), ')') as decr, id_empleado, 0 as haber, DATEDIFF(hasta, desde) + 1 as debe, desde
      FROM novedades n
