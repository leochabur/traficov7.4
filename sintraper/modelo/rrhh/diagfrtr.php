...
table thead tr .header {
	...
	cursor: pointer;
                        ...
                        <script type='text/javascript'>
                                ...
                                function cambiarFranco(emp, fec, st){
                                         ...
                                         $.post('/modelo/rrhh/diagfrtr.php', {accion:'update', id_e: emp, fecha: fec, estado: state});
                                }";
              $tabla.="</script>";
  ...
  elseif($accion == 'update'){
     ...
     if ($st){
        ...
        $values = "$conductor, '$fecha', '$fecha', 16, 'no_disp', 1, 0, $_SESSION[userid], now(), '', $_SESSION[structure]";
        print (insert('novedades', $campos, $values));
     }
     else{
          $conn = conexcion();
          $sql = "DELETE FROM novedades where (desde = '$fecha') and (id_empleado = $conductor) and (id_novedad = 16) and (id_estructura = $_SESSION[structure])";
