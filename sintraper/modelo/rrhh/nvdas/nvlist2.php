     ...
     elseif($accion == 'lnov'){
        ...
        if ($_POST['emple']){
           $empleado = "and (n.id_empleado = $_POST[emple])";
              ...
              $sql = "SELECT n.id_empleado, n.id, upper(concat(em.apellido, ', ',em.nombre)) as empleado, date_format(desde, '%d/%m/%Y') as desde, date_format(hasta, '%d/%m/%Y') as hasta, cn.nov_text, e.nombre, u.apenom
                      FROM novedades n
