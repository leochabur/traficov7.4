<?
  ...
  if (true){
    ...
    $sql="SELECT o.*
          FROM(	(SELECT interno, upper(c.razon_social)as razon_social, 0 as id_cond, fservicio as fsrv, upper(servicio) as servicio, date_format(hsalida, '%H:%i') as hsalida, o.id, date_format(fservicio, '%d/%m/%Y') as fservicio, date_format(hcitacion, '%H:%i') as hcitacion, '                  ORDENES A DESIGNAR' as apellido, 0 as legajo, date_format(hfinservicio, '%H:%i') as hfinservicio, comentario
                         FROM (SELECT  nombre as servicio, hsalida, id, id_chofer_1, fservicio, hcitacion, id_cliente, hfinservicio, id_estructura_cliente, id_micro, comentario
			       FROM ordenes
                ...
                UNION
			(SELECT interno, upper(c.razon_social)as razon_social, o.id_chofer_1 as id_cond, fservicio as fsrv, upper(servicio) as servicio, date_format(hsalida, '%H:%i') as hsalida, o.id, date_format(fservicio, '%d/%m/%Y') as fservicio, date_format(hcitacion, '%H:%i') as hcitacion, upper(if(emp.id = 1,concat(ch1.apellido, ', ',ch1.nombre), concat('(',emp.razon_social,') ', ch1.apellido, ', ',ch1.nombre))) as apellido, legajo, date_format(hfinservicio, '%H:%i') as hfinservicio, comentario
                         FROM (SELECT  nombre as servicio, hsalida, id, id_chofer_1, fservicio, hcitacion, id_cliente, hfinservicio, id_estructura_cliente, id_micro, comentario
			       FROM ordenes
        ...
        UNION
			(SELECT interno, upper(c.razon_social)as razon_social, o.id_chofer_2 as id_cond, fservicio as fsrv, upper(servicio) as servicio, date_format(hsalida, '%H:%i') as hsalida, o.id, date_format(fservicio, '%d/%m/%Y') as fservicio, date_format(hcitacion, '%H:%i') as hcitacion, upper(if(emp.id = 1,concat(ch2.apellido, ', ',ch2.nombre), concat('(',emp.razon_social,') ', ch2.apellido, ', ',ch2.nombre))) as apellido, legajo, date_format(hfinservicio, '%H:%i') as hfinservicio, comentario
                         FROM (SELECT  nombre as servicio, hsalida, id, id_chofer_2, fservicio, hcitacion, hfinservicio, id_cliente, id_estructura_cliente, id_micro, comentario
			       FROM ordenes
			       WHERE ((fservicio between '$desde' and '$hasta') and (id_estructura = $_SESSION[structure])) and (not suspendida) and (not borrada)
			       ) o
        ...
        UNION ALL
			(SELECT '' as interno, '' as razon_social, n.id_empleado as id_cond, '$desde' as fsrv, upper(concat(nov_text,'  (',date_format(desde, '%d/%m/%Y'),' - ',date_format(hasta, '%d/%m/%Y'),')')) as servicio,
			        '00:00' as hsalida, 0 as id, date_format('$desde', '%d/%m/%Y') as fservicio, '00:00' as hcitacion, upper(if(emp.id = 1,concat(em.apellido, ', ',em.nombre), concat('(',emp.razon_social,') ', em.apellido, ', ',em.nombre))) as apellido, legajo, '00:00' as hfinalizacion, '' as comentario
			 FROM (SELECT * FROM novedades WHERE ((('$desde' between desde and hasta) or ('$hasta' between desde and hasta)) and (id_estructura = $_SESSION[structure])) ) n
        ...
        $cond
        order by apellido, fsrv, hcitacion";
