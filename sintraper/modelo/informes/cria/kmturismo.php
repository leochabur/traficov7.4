<?php
  ...
  elseif($accion == 'reskm'){
     ...
     $sql = "select upper(razon_social) as cli, date_format(o.fservicio, '%d/%m/%Y') as fservicio, interno, upper(ori.ciudad), upper(des.ciudad), o.km, comentario , upper(concat(e1.apellido,', ', e1.nombre)) as emple1, upper(concat(e2.apellido,', ', e2.nombre)) as emple2, o.fservicio as fsrv, date_format(hsalida, '%H:%i') as hsalida, date_format(hllegada, '%H:%i') as hllegada
             from (SELECT fservicio, cr.id_cliente, cr.id_estructura_cliente, id_micro, ord.km, ord.nombre, ord.hsalida, ord.hfinservicio as hllegada, id_servicio, id_estructura_servicio, id_ciudad_origen, id_estructura_ciudad_origen, id_ciudad_destino, id_estructura_ciudad_destino, comentario, id_chofer_1, id_chofer_2
                   FROM ordenes ord
                   ...
                   union all
                   select fservicio, id_cliente, id_estructura_cliente, id_micro, km, nombre, hsalida, hfinservicio as hllegada, id_servicio, id_estructura_servicio, id_ciudad_origen, id_estructura_ciudad_origen, id_ciudad_destino, id_estructura_ciudad_destino, comentario, id_chofer_1, id_chofer_2
                   from ordenes ord
             ...
             order by razon_social, fsrv, o.hsalida";
     ...
     while ($data){
           ...
           while (($data) &&($int == $data['cli'])){
               ...
               $tabla.="<tr bgcolor='$color'>
                            ...
                            <td align='center'>$data[hsalida]</td>
                            <td align='center'>$data[hllegada]</td>
                            <td align='right'>$data[2]</td>
                            <td align='left'>".html_entity_decode($data[3])."</td>
                            <td align='left'>".html_entity_decode($data[4])."</td>
                            <td align='right'>$data[5]</td>
                            <td align='left'>".html_entity_decode($data[7])."</td>
                            <td align='left'>".html_entity_decode($data[8])."</td>
