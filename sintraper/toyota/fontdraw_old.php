<?php
  ...
  $sql = "select date_format(o.fservicio, '%d/%m/%Y'),
                 upper(o.nombre),
                 interno,
                 date_format(ho.hllegada,'%H:%i') as llegadiag,
                 date_format(o.hllegada,'%H:%i') as llegareal,
                 round((if (o.hllegada <= ho.hllegada,1,
                               if(
...
where o.fservicio between '$desde' and '$hasta' and ts.id = $t_ser and i_v = '$iv' and tu.id = $turno and o.id_cliente = 10 and not o.borrada and not o.suspendida
...
elseif($iv == 'v'){
  $sql = "select date_format(o.fservicio, '%d/%m/%Y'),
                 upper(o.nombre),
                 interno,
                 date_format(ho.hsalida,'%H:%i') as saldiag,
                 date_format(o.hsalida,'%H:%i') as salreal,
                 round((if (o.hsalida <= ho.hsalida,1,
                               if(
...
order by o.fservicio, o.hsalida";
  ...
  $sql = "select date_format(fservicio, '%d/%m/%Y'),
                 ...
                 round((cantpax/cantasientos)*100,2) as porefic,
                 os.comentario
from ordenes o
inner join servicios s on s.id = o.id_servicio and s.id_estructura = o.id_estructura_servicio
inner join tiposervicio ts on ts.id = s.id_TipoServicio and ts.id_estructura = s.id_estructura_TipoServicio
