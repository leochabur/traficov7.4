...
?>
 ...
 <script>
    ...
    <?php
      ...
      $result = mysql_query("SELECT id_empleado, concat(legajo,' - ', apellido,', ',nombre) as name FROM empleados WHERE (id_estructura = $_SESSION[structure]) and (activo) and (id_cargo = 1) ORDER BY apellido", $con);
      $i=0;
      while ($data = mysql_fetch_array($result)){?>
            choferes.push(new Array(<?php echo $data['id_empleado'];?>,  '<?php echo  "$data[name]";?>'));
      <?php
           $i++;
      }
         ...
         <div id="tablaordenes">
                            <?php
                                 ...
                                 $query = mysql_query("SELECT o.id, finalizada, o.borrada, o.km, date_format(o.hcitacion, '%H:%i') as hcitacion, date_format(o.hsalida, '%H:%i') as hsalida, o.nombre, concat(ch1.apellido, ', ',ch1.nombre) as chofer1, upper(c.razon_social) as razon_social, concat(ch2.apellido, ', ',ch2.nombre) as chofer2, comentario, interno, c.id as id_cli, d.ciudad, d.id as id_city, m.id as id_micro, ch1.id_empleado as id_emple_1
                                                       ...
                                                       WHERE (fservicio = '$fecha') and  (o.id_estructura = $_SESSION[structure])
                                                       ORDER BY c.razon_social, d.ciudad, o.hcitacion", $con);
