                                ...
                                <tr>
                                    ...
                                    <td>
                                        <select id="unidad" name="unidad" class="ui-widget-content  ui-corner-all"  validate="required:true">
                                                <?php
                                                    $sql = "select id, interno, 1 as orden, 'SINTRA' as prop
                                                            from unidades u
                                                            where id_propietario = 51 and activo
                                                            union
                                                            select id, interno, 2 as orden, 'MASTER BUS' as prop
                                                            from unidades u
                                                            where id_propietario = 1 and activo and id_estructura = 1
                                                            order by orden, interno";
                                                    $result = mysql_query($sql, $conn);
                                                    $row = mysql_fetch_array($result);
                                                    while ($row){
                                                        $prop = $row['prop'];
                                                        while (($row) && ($prop == $row['prop'])){
                                                            print "<option value='$row[id]'>$row[interno]</option>";
                                                            $row = mysql_fetch_array($result);
                                                        }
                                                        if ($row)
                                                            print "<option disabled>_________</option>";
                                                    }
                                                ?>
                                        </select>
                                    </td>
                                    ...
                                    <td>
                                        <select id="tipo" name="tipo" class="ui-widget-content  ui-corner-all"  validate="required:true">
                                                <?php
                                                  ...
                                                  $sql = "SELECT id, rubro FROM rubros_anomalias WHERE activo";
                                                  $result = mysql_query($sql, $conn);
                                                  $row = mysql_fetch_array($result);
                                                  while ($row){
                                                        print "<option value='$row[id]'>$row[rubro]</option>";
                                                        $row = mysql_fetch_array($result);
                                                  }
