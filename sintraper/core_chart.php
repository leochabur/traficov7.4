<?php
        ...
        if ($avanzada){
           if ($fil_srv){
              ...
              //     die("SELECT upper(ciudad) FROM ciudades WHERE id = $origen");
                   ...
                   if ($data = mysql_fetch_array($result))
                      $titulo= htmlentities($data[0])." $titulo";
                }
           }
        }
     ...
     function graphConfiabilidad($fdesde, $fhasta, $origen, $destino, $servicio, $t_serv, $turno, $i_v ,$avanzada, $fil_city, $city_origen, $iv, $fil_srv, $servicio){
        ...
        $sql="SELECT date_format(o.fservicio, '%d'), $calculo_conf, date_format(o.fservicio, '%d/%m/%Y'), o.fservicio as auxi, o.fservicio, day(o.fservicio), (month(o.fservicio)-1), year(o.fservicio)
              FROM (select id_estructura, id_servicio, id_estructura_servicio, hsalida, hllegada, fservicio, id from ordenes where (fservicio between '$desde' and '$hasta') and (not borrada) and (not suspendida) and (id_cliente = 10)) o
              INNER JOIN (select id_estructura, id_servicio, id_estructura_servicio, hsalida, hllegada, fservicio, id from horarios_ordenes) ho on ho.id = o.id";
