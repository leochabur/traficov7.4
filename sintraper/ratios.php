...
/*$export = mysql_connect('localhost', 'root', 'leo1979');
                     ...
                     $export = conexcion();
                     $sql="select tipo, turno, round(avg(confiabilidad),2) as conf, round(avg(eficiencia),2) as efic, sum(cantpax) as cantpax, count(*) as recorridos, count(distinct(id_micro)) as micros, sum(km) as km, tipo_serv
from (
SELECT date_format(o.fservicio, '%d/%m/%Y') as fecha, round((o.cantpax/cantasientos)*100,2) as Eficiencia,
       if (s.id_tipoServicio is null, 'Produccion', ts.tipo) as tipo, turno, o.id_servicio, o.cantpax,
...
cast(if (o.hllegadaplantareal <= o.hllegada,1,
  ...
  cast(if (o.hsalidaplantareal <= o.hsalida,1,
                               ...
                               if(
                                   (time_to_sec(timediff(o.hsalidaplantareal, o.hsalida))/60) < 20,
                                       (0.9-((time_to_sec(timediff(o.hsalidaplantareal, o.hsalida))/60)-5)*0.06),
                               0)))*100 as decimal(5,2)))) as Confiabilidad, o.km, o.id_micro as id_micro, ts.id as tipo_serv

FROM (select o.id, id_estructura_servicio, hsalidaplantareal, hllegadaplantareal, id_ciudad_origen, id_estructura_ciudad_origen, id_estructura_ciudad_destino, fservicio, cantpax, id_servicio,
...
//die($sql);
          ...
          /*  $pdf->SetFillColor(255, 255, 0);
            ...
            $sql="select *
                  from (
                       SELECT fservicio, count(distinct(id_micro)) as cant_micros, ts.tipo, tu.turno, id_TipoServicio
                       from (
                            select o.id, id_estructura_servicio, fservicio, cantpax, id_servicio, o.hllegada, o.hsalida, km, id_micro,
                                   if (id_servicio is null, (select id_turno from tipotnoordenes where id_orden = o.id), s.id_turno) as id_turno,
                                   o.id_estructura as id_estructura_Turno,
                                   if (id_servicio is null, (select id_tipo_servicio from tipotnoordenes where id_orden = o.id), s.id_tipoServicio) as id_TipoServicio,
                                   o.id_estructura as id_estructura_TipoServicio
                            from ordenes o
                            left join servicios s ON s.id = o.id_servicio and s.id_estructura = o.id_estructura_servicio
                            where month(o.fservicio) = $_GET[mes] and year(o.fservicio) = $_GET[anio] and o.id_estructura = $_SESSION[structure] and o.id_cliente = $_GET[cli] and not o.borrada and not o.suspendida) o
                       left JOIN tiposervicio ts ON ts.id = id_TipoServicio and ts.id_estructura = id_estructura_TipoServicio
                       left JOIN turnos tu ON tu.id = id_turno and tu.id_estructura = id_estructura_Turno
                       group by fservicio, id_TipoServicio, id_turno
                  ) o
                  where (tipo is not null) and (id_TipoServicio not in (16, 3))";
          //  die($sql);
            $result = mysql_query($sql, $export) or die(mysql_error($export));
