...
$query="SELECT fservicio as fecha, date_format(fservicio, '%d/%m/%Y') as fservicio, date_format(hcitacionreal, '%H:%i') as hcitacion, date_format(hsalidaplantareal, '%H:%i') as hsalida, date_format(hfinservicioreal, '%H:%i') as hfinserv, o.nombre, upper(c.razon_social) as razon_social, comentario, interno
        FROM ordenes o
...
else
{
    ...
    while($data){
         ...
         $est_diag="SELECT *
                    ...
                    where (fecha = '$data[fecha]') and (id_estado = 1)";
