...
$numero = cal_days_in_month(CAL_GREGORIAN, $_GET['mes'], $_GET['anio']);

$cond_fecha = "between DATE_SUB('$_GET[anio]-$_GET[mes]-01', INTERVAL 5 month) and '$_GET[anio]-$_GET[mes]-$numero'";
//die("numero dias ".$cond_fecha);
...
cast(if (o.hllegadaplantareal <= o.hllegada,1,
  ...
  cast(if (o.hsalidaplantareal <= o.hsalida,1,
      ...
      FROM (select o.id, id_estructura_servicio, hsalidaplantareal, hllegadaplantareal, id_ciudad_origen, id_estructura_ciudad_origen,
                   ...
                   if (tto.i_v is null, s.i_v, tto.i_v) as i_v, concat(year(o.fservicio),'-',date_format(o.fservicio,'%m')) as periodo
            from ordenes o
            ...
            where fservicio $cond_fecha and o.id_estructura = $_GET[str] and o.id_cliente = $_GET[cli] and not o.borrada and not o.suspendida and (hllegadaplantareal is not null || hsalidaplantareal is not null)) o
...
group by periodo, tipo, turno";  //month(o.fservicio) between 2 and $_GET[mes] and year(o.fservicio) = $_GET[anio]
         //    die($sql);
            $i=1;
     ...
     //       print_r($mese_compara);
       //     die("cantidad ".count($mese_compara));
            ...
            $pdf->ln();
            $posx = $pdf->getX();
            $pdf->setXY($posx+55,$pdf->getY());
            foreach ($mese_compara as $clave => $valor){
                    $pdf->Cell(18,7, $clave, 0, 0, 'R', false);
            }
            $pdf->ln();
            ...
            foreach ($mese_compara as $clave => $valor){
                   ...
                   $pdf->Cell(18,7, number_format($conf,2), 0, 0, 'R', false);
            }
            ...
            foreach ($mese_compara as $clave => $valor){
                   ...
                   // $pdf->Cell(20,7, $clave, 0, 0, 'R', false);
                   $pdf->Cell(18,7, number_format($efic,2), 0, 0, 'R', false);
            }
         ...
         //   $pdf->Cell(20,7, number_format($efic,2),0, 0, 'R', false);
            ...
            $aux_claves = array();
            $max_cant_tipos = 0;
            foreach ($mese_compara as $key => $value){      //controla la cantidad de tipos de servicios por si difiere de un periodo a otro
                    if (count($value[0]) > $max_cant_tipos)
                       $max_cant_tipos = count($value[0]);
            }
            $i=0;
            $despx=0;
            foreach ($mese_compara as $key => $value){
                    ...
                    foreach ($value[0] as $clave => $valor) {       ///imprime la cant de servicios
                            ...
                            if (!array_key_exists($clave, $aux_claves)){
                               $pdf->Cell(150,7,$clave,0, 0, 'L', false);
                               $aux_claves[$clave]=$i;
                               $i+=7;
                            }
                            $pdf->SetXY($posx+55+$despx, $auy+$aux_claves[$clave]);
                            $pdf->Cell(18,7,$valor,0, 0, 'R', false);
                            $tot_recos+=$valor;
                            $pdf->ln();
                    }
                    ////imprime total de servicis
                 //   $pdf->Cell(150,7,'',0, 0, 'L', false);
                    $pdf->SetX($aux+55+$despx);
                    $pdf->Cell(18,7,$tot_recos,1, 0, 'R', false);
                    $despx+=18;
            }
            ...
            $max_cant_tipos = 0;
            foreach ($mese_compara as $key => $value){      //controla la cantidad de tipos de servicios por si difiere de un periodo a otro
                    if (count($value[1]) > $max_cant_tipos)
                       $max_cant_tipos = count($value[1]);
            }
            $i=0;
            $despx=0;
            foreach ($mese_compara as $key => $value){
                    ...
                    foreach ($value[1] as $clave => $valor) {       ///imprime la cant de servicios
          ...
          /*  foreach ($paxs as $clave => $valor) {       ///imprime la cant de servicios
            ...
            foreach ($mese_compara as $clave => $valor){
                   ...
                   // $pdf->Cell(20,7, $clave, 0, 0, 'R', false);
                   $pdf->Cell(18,7, $km, 0, 0, 'R', false);
            }
        //    $pdf->Cell(20,7,$km,0, 0, 'R', false);
            $pdf->ln();
       //     $pdf->ln();//recuperar .inea
...
from (
    ...
    from (
          ...
          left join (select * from servicios where id_estructura = $_GET[str]) s on s.id = o.id_servicio
          left join tipotnoordenes tto on tto.id_orden = o.id
          where fservicio $cond_fecha and o.id_estructura = $_GET[str] and o.id_cliente = $_GET[cli] and not o.borrada and not o.suspendida
          ) o
    left JOIN (Select * from tiposervicio where (id_estructura = $_GET[str]) and (id not in (16, 3))) ts ON ts.id = id_tipo_servicio
...
where tipo is not null";
          //  die($sql);
$sql="select fservicio, cant_micros, tipo, turno, id_tipo_servicio, periodo
from (
    SELECT fservicio, count(distinct(id_micro)) as cant_micros, ts.tipo, tu.turno, id_tipo_servicio, periodo
    from (
          select o.id, id_estructura_servicio, fservicio, cantpax, id_servicio, o.hllegada, o.hsalida, km, id_micro,
                 if (tto.id_turno is null, s.id_turno, tto.id_turno) as id_turno,
                 if (tto.id_tipo_servicio is null, s.id_TipoServicio, tto.id_tipo_servicio) as id_tipo_servicio,
                 if (tto.i_v is null, s.i_v, tto.i_v) as i_v,
                 concat(year(fservicio),'-',date_format(fservicio, '%m')) as periodo
          from (select id, id_estructura_servicio, fservicio, cantpax, id_servicio, hllegada, hsalida, km, id_micro from ordenes where fservicio $cond_fecha and id_estructura =  $_GET[str] and id_cliente = $_GET[cli] and not borrada and not suspendida) o
          left join (select id, id_estructura, id_turno, id_TipoServicio, i_v  from servicios where id_estructura = $_GET[str]) s on s.id = o.id_servicio
          left join tipotnoordenes tto on tto.id_orden = o.id
          ) o
    left JOIN (Select * from tiposervicio where (id_estructura = $_GET[str]) and (id not in (16, 3))) ts ON ts.id = id_tipo_servicio
    left JOIN turnos tu ON tu.id = id_turno
...
order by fservicio";
   ...
   //         $pdf->Cell(150,7,'MICROS UTILIZADOS SIMULTANEAMENTE',0, 0, 'L', false);         //recupear esta linea
            $pdf->ln();
            $pdf->SetFont('Times','',10);
            $data = mysql_fetch_array($result);
            $micros=array();
            ...
            while ($data){
                  ...
                  $meses_compara[$periodo] = $micros;
                  while ($data && ($periodo = $data['periodo'])){
                        $tipo = $data['tipo'];
                        $turno=$data['turno'];
                        if (!isset($meses_compara[$periodo][$tipo])){
                           $meses_compara[$periodo][$tipo]=array();
                        }
                        if (!isset($meses_compara[$periodo][$tipo][$turno])){
                           $meses_compara[$periodo][$tipo][$turno]=0;
                        }
                        if ($data['cant_micros'] > $meses_compara[$periodo][$tipo][$turno]){
                           $meses_compara[$periodo][$tipo][$turno] = $data['cant_micros'];
                        }
                        $data = mysql_fetch_array($result);
                  }
            }
         //   print_r($meses_compara);
            foreach ($micros as $clave => $valor) {
                    foreach ($valor as $clave_tu => $valor_tu) {
                            ...
                            $pdf->Cell(18,7,$valor_tu,0, 0, 'R', false);
                           // $tot_pax+=$valor;
                            $pdf->ln();
                    }
            }
            ...
            ///////
          //  $pdf->ln();recuperr esta linea
            ///antiguedad media parque
            $pdf->SetFont('Times','B',10);
            $sql="SELECT periodo, round(sum(anio)/count(*)) as media
                  from ( select id_micro, concat(year(fservicio),'-',date_format(fservicio,'%m')) as periodo
                         from ordenes o
                          where o.fservicio $cond_fecha and o.id_estructura = $_GET[str] and o.id_cliente = $_GET[cli] and not o.borrada and not o.suspendida) o
            ...
            while ($data = mysql_fetch_array($result))
                  $pdf->Cell(18,7, $data[1], 0, 0, 'R', false);
