<?php
     ...
     //, convert(VARCHAR, fecha, 103) as fec, convert(VARCHAR, fecha, 108) as hora
     // $result = mssql_query ("select * from movilesmaster where movil = 'int - $int'", $link) or die (mssql_get_last_message());
     $result = mssql_query ("select * from movilesmaster", $link) or die (mssql_get_last_message());
     ...
     for ($i = 0; $i < mssql_num_fields($result); ++$i) {
       ...
       // Print the row
       $tabla.= '<tr>';
       $tabla.= '<td>' . $field->name . '</td><br>';
       $tabla.= '<td>' . strtoupper($field->type) . '</td><br>';
       $tabla.= '<td>' . $field->max_length . '</td><br>';
       $tabla.= '</tr>';
     }
     die($tabla);
