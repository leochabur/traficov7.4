...
<?php
        ...
        try{
                ...
                $sql = "SELECT c.id, upper(c.nombre) as nombre, gpx.id as gpx
                        ...
                        where (id_cliente = 161 or (c.nombre like '%rondin%' and id_cliente = 13)) and c.id_estructura = 1 and c.activo and s.i_v = 'i' and not vacio and s.activo
                        group by c.id
                        order by c.nombre";
    ...
    <nav class="navbar navbar-light bg-light">
        <form enctype="multipart/form-data" class="text-right mt-4" method="POST" action=".">
            <div class="form-row mb-3">
                <span class="h5">HONDA MOTOR ARGENTINA SA</span>
            </div>
            <div class="form-row mb-3">
            ...
            <select class="form-control col-lg-12" name="cronograma">
                ...
                <?php
                        while ($row = mysqli_fetch_array($result))
                        {
                            ...
                            print "<option class='$class' value='$row[id]'>". htmlentities($row[nombre])."</option>";
                        }
                ?>
            </select>
        </div>
            ...
            <?php
                ...
                while ($gpx = mysqli_fetch_array($files))
                {
                    print '<li class="list-group-item">
                            <input data-id="'.$gpx['id'].'" data-file="'.$gpx['archivo'].'" type="button" class="btn btn-sm btn-danger delete" value="X"/> - '.htmlentities($gpx['nombre']) .'    /    '.htmlentities($gpx['archivo']).'</li>';
                }
