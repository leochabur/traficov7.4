          ...
          // $ult = '';
           while ($i = mysql_fetch_array($result)){
                   // if ($i == '')
                       $data[$i[0]]=$i[1];
                /*    else{
                         $dias = mysql_query("select  DATEDIFF('$i[2]', '$ult')", $conn);
                         $d = mysql_fetch_array($dias);
                         for ($j = 1; $j <= $d[0]; $j++){
                             $auxult = $i[2];
                             $auxd = mysql_query("SELECT DATE_ADD('$auxult', INTERVAL 1 DAY)", $conn);
                             $raux = mysql_fetch_array($auxd);
                             $data[]
                         }
                    }
                    $ult=$i[3];   */
           ...
           $sql="SELECT date_format(fservicio, '%d'), round((if (o.hsalida <= s.hsalida,1,
                               if(
    ...
    <br><br>
         <form id="upuda" method="POST">
	           <fieldset class="ui-widget ui-widget-content ui-corner-all">
                         ...
                         <br>
                         <div align="center"><h2><p align="center"><?php echo $title."<br><hr><br>".htmlentities($tg);?></p></h2>
                              <ul>
                                  <li><a href="#tabs-1">Nunc tincidunt</a></li>
                                  <li><a href="#tabs-2">Proin dolor</a></li>
                              </ul>
                              <div id="tabs-1">
                                   <div id="<?php ?>" align="center" ><?php echo $chart;?><br><?php if (isset($_POST['cargar'])){?><input type="button" id="<?php echo " $desde $hasta $turno $t_serv"."i";?>" value="Ver Datos"><?php }?></div>
                              </div>
