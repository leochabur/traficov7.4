<?php
  ...
  for ($i = 0; $i < 15; $i++)
  {
      ...
      //la fecha que debe diagramar es un feriado, debe levantar las ordenes del ultimo domingo
      {
        ...
        $ordenesBase = getOrdenesDiagramadas($fechaAux->format('Y-m-d'), 1, $conn); //recupera todas las ordenes llamadas base, se toma como que esas ordenes son las que deberan estar diagramadas (Para el caso son las ordenes de 14 dias atras)
      }
      else
      {
        $fechaAux = clone $fechaConsulta;
      ...
      $ordenesFechaDiagrama = getOrdenesDiagramadas($fechaInicio->format('Y-m-d'), 1, $conn); //son las ordenes del dia que debe generar las ordenes, con esto verifica si las mismas ya no han sido diagramadas
  ...
  function getOrdenesDiagramadas($fecha, $str, $conn)
  {
      $sql = "SELECT o.id as idOrden,
              ...
              inner join (select id_servicio, id_estructura_servicio, id, id_micro, fservicio, id_chofer_1
                          from ordenes
                          where id_estructura = $str AND fservicio = '$fecha' and id_cliente = 13 and not borrada and not suspendida) o ON id_servicio = s.id AND id_estructura_servicio = s.id_estructura
