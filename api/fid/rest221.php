    ...
    try
    {
       /* $request = $app->request();
        ...
        $sql = "SELECT upper(concat(apellido,',', nombre)) as nombre, replace(nrodoc,'.','') as nrodoc, e.id_empleado as id, 
                        if (embedding is null, '', embedding) as embedding, e.activo FROM empleados e LEFT JOIN embeddings em ON em.id_empleado = e.id_empleado WHERE ((activo) and (stamp >= $stamp)) or ((fechaHoraBaja >= $stamp) and not activo) $type ORDER BY e.id_empleado";
...
$app->post('/record', function () use ($app) 
{
    try
    {
        ...
        if (!is_array($input))
        {
            echoResponse(200, array('error' => true, 'message' => 'Los parametros recibidos son incorrectos'));
        }
        else
        {
            if (array_key_exists('id', $input) && array_key_exists('stamp', $input) && array_key_exists('action', $input))
            {
                if (is_numeric($input['stamp']))
                {
                    ...
                    $sql = "INSERT INTO accesosregistro (id_empleado, stamp, sentido) 
                            VALUES ($input[id], $input[stamp], $input[action])";
                    mysqli_query($conn, $sql);
                    ...
                    else
                    {
                        ...
                        mysqli_close($conn);
                        echoResponse(200, array('error' => true, 'message' => $error));
                    }
                }
                else
                {
                    echoResponse(200, array('error' => true, 'message' => 'El valor del campo stamp es incorrecto'));
                }
            }
            else
            {
                echoResponse(200, array('error' => true, 'message' => 'Todos los parametros (id, stamp y action) son requeridos'));
            }
        }   
    }
    catch(Exception $e){echoResponse(200, array('error' => true, 'message' => $e->getMessage()));}
