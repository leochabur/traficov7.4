...
require_once ('../../vista/segvial/lib/nusoap.php');
...
function calcularHorarioCierre($fechita, $bus)
{
    try
    {
            $response = array();
...
$app->post('/integracion/eventos', function () use ($app) 
{
        try
        {
            $request = $app->request();
            $body = $request->getBody();
            $insert = "INSERT INTO notificacionesUrbetrack (fechaHora, body) VALUES (now(), '$body')";
            ejecutarSQLPDO($insert);

          /*  $request = $app->request();
            $body = $request->getBody();
            $input = json_decode($body, true); */

         //   calcularHorarioCierre($input['Fecha'], $input['Interno']);
            echoResponse(200, ['message' => 'ok']);
        }
        catch (Exception $e){
            echoResponse(200, ['message' => $e->getMessage()]);
        }
