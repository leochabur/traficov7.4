...
else
{
    print "<hr>
        <center>
        <h2><a href='https://docs.google.com/forms/d/e/1FAIpQLSfzjTTceoM7qSIMy7NvHbGcPLrVKe2G08777WUK-iSPgKQ9ww/viewform' target='_blanck'><font class='blink' color='red'>ENCUESTA DE SATISFACCION DEL PERSONAL - AÑO 2020</font></a></h2></center>";
    print "<HR>";
    ...
    $est_diag="SELECT ed.estado, e.id_estado
               ...
               where (e.id_estructura = 1) and (e.id_estado = 1) and (e.id_estado = ed.id) and (e.fecha between curdate() and ADDDATE(curdate(),2))";

    while($data){
         $ult = $data['fservicio'];
         $est_diag="SELECT *
                    FROM estadoDiagramasDiarios e
                    where (fecha = '$data[fecha]') and (id_estado = 1) and (id_estructura = 1)";
