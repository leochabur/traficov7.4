...
$conexion = conexcion();
$query = mysql_query("SELECT id_empleado, id_cargo, Concat(Upper(apellido),', ', Upper(nombre)) as name, email 
                      FROM empleados 
                      WHERE (legajo = '$_POST[txtNombre]') and (activo = 1) and (id_empleador = 1)") or die(mysql_error());
...
else
{
    ...
    $sql = "SELECT id_empleado, Concat(Upper(apellido),', ', Upper(nombre)) as name, upper(razon_social) as emple, REPLACE(nrodoc,'.','') as dni, emp.id as idEmpleador
            FROM empleados e
    ...
    { //Verifica que el chofer no se encuentre deshabilitado
      ...
      {  
                       ...
                       $_SESSION['super'] = $data["super"];
                       $_SESSION['str'] = $data["estructura"];      
                       $_SESSION['id_empleador'] = $data["idEmpleador"];             
