<?php
  ...
  function getSQLConductores($fecha, $estructura){
             $sql = "SELECT *
                   ...
                   where ('$fecha' between desde and hasta) and (e.activo) and (not e.borrado) and (id_cargo in (1,2,3,4)) and (n.activa) and (e.id_estructura = $estructura)
                   )
                   union
                   ...
                   from empleados e
                   inner join empleadores em on em.id = e.id_empleador
                   where (e.activo) and (not e.borrado) and (e.id_estructura = $estructura) and (id_cargo in (1,2,3)) and (e.id_empleado not in (SELECT e.id_empleado
                                                                                                            from novedades nov
                                                                                                            inner join empleados e on e.id_empleado = nov.id_empleado
                                                                                                            where ('$fecha'  between desde and hasta) and (e.activo) and (not e.borrado) and (nov.activa) and (id_cargo in (1,2,3,4))
...
from (select id_empleado, id_empleador, apellido, nombre from empleados where (activo) and (not borrado) and (id_cargo in (1,2,3,4)) and (id_estructura = $estructura)) e
...
order by id_nov, emple";
  ...
  function getSQLConductoresEstructura($estructura){
             $sql = "SELECT id_empleado, if(em.id = 1,upper(concat(e.apellido, ', ',e.nombre)), upper(concat('(',em.razon_social,') ', e.apellido, ', ',e.nombre))) as apenom
                    FROM empleados e
                    inner join empleadores em on em.id = e.id_empleador
                    WHERE (e.id_estructura = $estructura) and (id_cargo in (1,2,3,4)) and (e.activo) and (not borrado)
