<?
  ...
  if ($accion == 'cls'){
   ...
   if ($_SESSION['structure'] == 1){
     $conn = conexcion();
     $sql = "select o.id_cliente, id_micro, upper(nombre) as nombre, date_format(fservicio, '%d/%m/%Y') as fecha, upper(razon_social) as cliente
             from ordenes o
      ...
      if($row['id_cliente'] != 13){
        ...
        if (!mysql_num_rows($result)){
              ...
              if ($row = mysql_fetch_array($result)){
                 ...
                 enviarMail("leochabur@gmail.com,mdepeon@masterbus.net,rpizzutti@masterbus.net,raguiar@masterbus.net,ealmada@masterbus.net,ddanchuk@masterbus.net" , $cuerpo, "Conflictos con la orden $id");
              }
        }
        }
     }
