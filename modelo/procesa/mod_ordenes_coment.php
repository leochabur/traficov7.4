...
error_reporting(0);
  ...
  include('../../modelsORM/src/ObservacionOrden.php');
...
elseif ($accion == 'check'){
 ...
 //    die('toronjin');
     $sql = "INSERT INTO obsSupervisores (id_orden, id_usuario, fecha_accion, comentario) VALUES ($clave, $_SESSION[userid], now(), '$valor')
  ...
  elseif($accion == 'upComment'){
    try{
        $orden = getOrden($_SESSION['structure'], $_POST['orden']);
        if (!$orden)
          print json_encode(array('status' => false, 'msge' => 'No se encontro la orden!!'));
