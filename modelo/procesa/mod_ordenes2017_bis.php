<?
  ...
  if ($accion == 'cls'){
     ...
     $conn = conexcion();
     $sql = "select id_cliente, id_micro, upper(nombre) as nombre, date_format(fservicio, '%d/%m/%Y') as fecha from ordenes where id = $id";
     ...
     if ($row = mysql_fetch_array($result)){
        ...
        $sql = "select id_coche, interno, 1 as disponible
                from (
                     ...
                     where permitido and r.id_cliente = $row[id_cliente] and (if (ant_max is null, true ,(year(now()) - anio) <= ant_max)) and activo ) o
        ...
        if (!mysql_num_rows($result)){
              ...
              $result = mysql_query($sql, $conn);
              if ($row = mysql_fetch_array($result)){
                 $cuerpo = "El interno $row[interno], que realizo el servicio $nombre, el dia $fecha, no se ajusta a los parametros solicitados por el cliente debido a:";
                 ...
                 enviarMail("dscirocco@masterbus.net,leochabur@gmail.com,mdepeon@masterbus.net,rpizzutti@masterbus.net,raguiar@masterbus.net,ealmada@masterbus.net,ddanchuk@masterbus.net", $cuerpo, "Conflictos con la orden numero $id");
              }
        }
      /*  else{
                           enviarMail("leochabur@gmail.com", "dentro de los parametros", "Ordenes aceptadas");
        }  */
