...
elseif ($accion == 'check'){
     ...
     print update('ordenes', $campo, $value, "(id = $id) and (id_estructura = ".STRUCTURED.")");
                  ...
                  if ($_POST['cmp'] == 'id_micro')
                  {
                      ...
                      {
                            $sqlDiagrama = "SELECT * FROM estadoDiagramasDiarios e where fecha = '$row[fservicio]' and id_estado = 1 and id_estructura = 1";
                            $resultDiagrama = mysql_query($sqlDiagrama, $conn);
                            if ($data = mysql_fetch_array($resultDiagrama))
                            { 

                                  $interno = "NULL";
                                  if($_POST['valor'])
                                  {
                                    $sqlMicro = "SELECT interno FROM unidades WHERE id = $_POST[valor]";
                                  ...
                                  curl_setopt_array($curl, array(
                                    CURLOPT_URL => "https://admtickets.masterbus.net/api/integrations/traffic/trips/".$_POST['clave'],
                                  ...
                                  curl_close($curl);      
                                  $sql = "INSERT INTO estadocomunicaciones (fecha, orden, estado) VALUES (now(), $_POST[clave], $result)";
