...
//
  ...
  //  if (isset($entityManager))
    ...
    // die ("busco tipo");
     ...
     $accion->setFechaAlta(new DateTime());
     if (!is_array($emples)){
        $response[error] = true;
  ...
  elseif($accion == 'tpos'){
      ...
      $tiposL = $entityManager->createQuery("SELECT t 
                                             FROM TipoAccionUnidad t 
                                             WHERE t.comenta = :comenta AND t.edilicio = :edilicio")
  ...
  elseif($accion == 'sveLvdEd'){
    $response = Array();
    $response['status'] = false;

    if ($_POST['emples'] == 'null'){
          $response['mje'] = 'No ha seleccionado ningun emleado para la tarea!!!!';
          print json_encode($response);
          exit();
    }
    if (!$_POST['tiempo']){
          $response['mje'] = 'No ha ingresado un tiempo para la tarea!!!!';
          print json_encode($response);
          exit();
    }
    if (!$_POST['fecha']){
          $response['mje'] = 'No ha seleccionado una fecha para la tarea!!!!';
          print json_encode($response);
          exit();
    }
    
    try{
        $acc = new AccionUnidad();
        $sector = find('Sector',$_POST['sector']);
        $acc->setSector($sector);

        $tiempo = DateTime::createFromFormat('H:i', $_POST['tiempo']);
        $acc->setTiempo($tiempo);

        $fecha = DateTime::createFromFormat('d/m/Y', $_POST['fecha']);
        $acc->setFecha($fecha);

        $tAccion = find('TipoAccionUnidad', $_POST['tipoAccion']);
        $acc->setAccion($tAccion);
        $acc->setFechaAlta(new DateTime());

        $emples = explode(',', $_POST['emples'] );

        foreach($emples as $emple){
           $empleado = find('Empleado', $emple);
           $acc->addResponsable($empleado);
        }
