<?php
  ...
  //   die($sql);
     ...
     while ($data){
               ...
               $tabla.="<tr id='$data[7]' $class>
                            ...
                            <td align='center'>".($data['orden_trabajo']?$data['orden_trabajo']:'')."</td>
  ...
  elseif($accion == 'modanom'){
                 ...
                 try
                 {
                    $desc = str_replace(',',' ', $_POST['otaller']);
                    $ides = $_POST['anomalia'];

                    if (isset($_POST['ASOC']))
                    {
                        foreach($_POST['ASOC'] as $selected)
                        {
                           $ides.= ','.$selected;
                        }
                    }
                    ...
                    update("anomalias", 
                           "reparada, observacion_taller, fecha_reparacion, hora_reparacion, id_usuario_reparacion, orden_trabajo", 
                           "$repa, '$desc', $fecha, $hora, $_SESSION[userid], '$_POST[orepa]'", 
                           "(id in ($ides))");              
                    print "1";
                 }
                 catch (Exception $e) {
