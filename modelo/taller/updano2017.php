<?
  ...
  if($accion == 'sveanom'){ //codigo para guardar una anomalia
             ...
             try{
                 $ok = insert('anomalias', 'id, id_usuario_alta, causa, id_unidad, id_rubroanomalia, fecha, detalle_anomalia, fecha_alta',
                              "$_SESSION[userid], $_POST[origen], $_POST[internos], $_POST[tipo], '$fecha', '$_POST[desc]', now()"); //agrega un conductor pendiente de procesamiento por parte del sector de RRHH
                 $sql = "SELECT date_format(fecha, '%d/%m/%Y'), interno, upper(origen), upper(apenom), upper(detalle_anomalia)
                         FROM anomalias a
