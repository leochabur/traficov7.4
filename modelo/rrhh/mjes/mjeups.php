<?php
     ...
     if ($accion == 'lcr'){
        ...
        $sql="SELECT em.id as id_empleador, upper(es.nombre) as str, e.id_empleado, legajo, upper(concat(apellido, ', ',e.nombre)) as apenom, nrodoc, if(e.activo, 'checked', '') as activo, upper(codigo) as codigo
                ...
                where (id_cargo = 1) and (e.id_empleador = $_POST[cli]) and (e.id_estructura in (SELECT uxe.id_estructura
                                                                                                 FROM usuarios u
        ...
        $tabla = '<br>Listado de personal<br><table id="example" name="example" align="left">
	                      <thead>
		                         <tr>
                                     <th>Legajo</th>
        ...
        $tabla.= '</table>
                  ...
                  <script>
                                ...
                                $("#example tr").click(function(){
                                                                  ...
                                                                  $.post("/modelo/rrhh/mjes/mjeups.php",{emple: id_emple, empledor:id_emplor,  nomcron: n_cron, accion: "lsrv"},function(data){
                                                                                                                                                           $("#srvcron").html(data);
                                                                                                                                                         });
                                                                  });
                  </script>';
     ...
     elseif ($accion == 'lsrv'){
        ...
        /*    $cond = ""; //verifica que modulos esta habilitado a operar el usuario
            ...
            $sql = "SELECT id, upper(mensaje), fecha_alta FROM mensajes where id_empleado = $emple order by fecha_alta desc";
            $result = mysql_query($sql, $conn) or die (mysql_error($conn));
            ...
            while ($data = mysql_fetch_array($result)){
              $tabla.="<option value='$data[0]'>".htmlentities($data[1])."</option>";
            }
            mysql_free_result($result);
            mysql_close($conn);
            $tabla.= '</td>
     ...
     elseif ($accion == 'savenv'){
            ...
            $sql = "SELECT id as id_orden
                    ...
                    WHERE (concat(fservicio,' ', hsalida) > now())";
