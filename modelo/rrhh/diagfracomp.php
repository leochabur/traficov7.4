  ...
  if ($accion == 'sbana'){ //codigo para generar sabana francos
     ...
     $sql="SELECT n.id_empleado, cn.id, desde
                    ...
                    where (desde between '$desde' and '$hasta') and (hasta between '$desde' and '$hasta') and
                          (e.id_estructura = $_SESSION[structure]) and (cn.id = 49)
...
table thead tr .header {
	...
	cursor: pointer;
                        ...
                        <script type='text/javascript'>
                                ...
                                function cambiarFranco(emp, fec, st){
                                         ...
                                         $.post('/modelo/rrhh/diagfracomp.php', 
  ...
  elseif($accion == 'update'){
     ...
     if ($st)
     {
        ...
        $values = "$conductor, '$fecha', '$fecha', 49, 'no_disp', 1, 0, $_SESSION[userid], '$fechaAlta', '', $_SESSION[structure]";
        try
        {
            $id = insertPDO('novedades', $campos, $values);
            ...
            if ($id)
            {
                ...
                $debito->setDescripcion($novText->getTexto());
          ...
          $conn = conexcion();
          $sql = "DELETE FROM novedades where (desde = '$fecha') and (id_empleado = $conductor) and (id_novedad = 49) and (id_estructura = $_SESSION[structure])";
