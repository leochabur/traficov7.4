...
elseif($accion == 'dexc'){
     $desde = dateToMysql($_POST['desde'], '/');
     $hasta = dateToMysql($_POST['hasta'], '/');
$sql="SELECT 'NOVEDAD' as causa, date_format(desde, '%d/%m/%Y') as desde, date_format(hasta, '%d/%m/%Y') as hasta, upper(nov_text) as detalle, null as reso
FROM novedades n
inner join cod_novedades cn on cn.id = n.id_novedad
...
union
SELECT 'SINIESRTO' as causa, date_format(fecha_siniestro, '%d/%m/%Y') as desde, date_format(fecha_siniestro, '%d/%m/%Y') as hasta, cast(concat('SINIESTRO OCURRIDO EN: ', upper(ciudad), ' CON EL INTERNO: ', interno) as char) as detalle, null as reso
FROM siniestros s
...
union
SELECT 'INFRACCION' as causa, date_format(fecha, '%d/%m/%Y') as desde, date_format(fecha, '%d/%m/%Y') as hasta, upper(infraccion) as detalle, null as reso
FROM infracciones i
...
union
SELECT 'PEDIDO DE EXPLICACION' as causa, date_format(fecha_emision, '%d/%m/%Y') as desde, date_format(fecha_emision, '%d/%m/%Y') as hasta, descripcion_hecho as detalle, rs.descripcion as reso
FROM descargos d
