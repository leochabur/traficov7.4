    ...
    //  $fin = DateTime::createFromFormat('Y-m-d', "$_POST[anio_h]-$_POST[mes_h]-25");   
      ...
      try
      {
          $q = $entityManager->createQuery("SELECT m
                                            ...
                                            WHERE m.fecha <= :hasta AND cat.id = 1 AND m.activo = :activo AND str.id = 1 AND emp.id = :empleador
                                            ORDER BY e.apellido");
          $q->setParameter('empleador', $_POST['empleador'])
            ->setParameter('activo', true);
          $q->setParameter('hasta',  $hasta->format('Y-m-d'));
       ...
       $select.="</select>";
          ...
          foreach ($resumen as $key => $res)
          {
                ...
                $tabla.="<tr>
                             ...
                             <td>".$res['e']."</td>
                             <td title='Click para ver detalle...' class='vff' data-emple='$res[e]' data-idemp='".$res['e']->getId()."' align='center'>".$res['frt']."</td>
                             <td title='Click para ver detalle...' class='vff' data-emple='$res[e]' data-idemp='".$res['e']->getId()."' align='center'>".$res['fet']."</td>
                             <td title='Click para ver detalle...' class='' data-emple='$res[e]' data-idemp='".$res['e']->getId()."' align='center'><b>".($res['fr'] - $res['frt'])."</b></td>
                             <td title='Click para ver detalle...' class='vff' data-tipo='adelan' data-emple='$res[e]' data-idemp='".$res['e']->getId()."' align='center'><b>".count($francosAadelantados[$key])."</b></td>
  ...
  elseif ($accion == 'detview')
  {
      ...
      else
      {
        $q = $entityManager->createQuery("SELECT m
                                            ...
                                            JOIN e.estructura str
                                            JOIN e.categoria cat
                                            WHERE m.fecha <= :hasta AND e = :empleado AND m.activo = :activo 
                                            ORDER BY m.fecha");
          $q->setParameter('activo', true)
            ->setParameter('empleado', $empleado);
