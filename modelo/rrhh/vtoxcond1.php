<?
  ...
  if($accion == 'load'){
          $sql="SELECT id_empleado as id, legajo, upper(concat(apellido, ', ',nombre)) as apenom,
                       (SELECT id_licencia FROM licenciasxconductor where (id_licencia = 1) and (id_conductor = id_empleado)) as nac,
                       (SELECT id_licencia FROM licenciasxconductor where (id_licencia = 2) and (id_conductor = id_empleado)) as prov,
                       (SELECT id_licencia FROM licenciasxconductor where (id_licencia = 3) and (id_conductor = id_empleado)) as mun,
                       (SELECT id_licencia FROM licenciasxconductor where (id_licencia = 4) and (id_conductor = id_empleado)) as alc,
                       (SELECT id_licencia FROM licenciasxconductor where (id_licencia = 5) and (id_conductor = id_empleado)) as pre
               FROM empleados e
          ...
          while($data = mysql_fetch_array($result)){
                      ...
                      $tabla.="<td align='center'><input type=\"checkbox\" $sel onClick=\"cambioEstado($data[id], 5, this.checked);\"></td></tr>";
          }
          $tabla.='</tbody>
                  </table>
                  <style type="text/css">
