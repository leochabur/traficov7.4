     ...
     elseif($accion == 'cargarCert'){
      ...
      elseif($accion == 'svecert'){
        ...
        try{
            ...
            insert("certmedicos", $campos, $values, $conn);
            commit($conn);
            $response[msge] = "Se ha almacenado con exito el certificado!!!";
            cerrarconexcion($conn);
            print json_encode($response);
        }
        catch (Exception $e) {
                           $response[status] = false;
                           $response[msge] = $e->getMessage()." - Se ha producido un error al intentar guardar el certificado!!!";
                           cerrarconexcion($conn);
                           print json_encode($response);
                          };
      }
