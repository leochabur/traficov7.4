<?php
  ...
  elseif($accion == 'ldsrv'){
     ...
     $sql = "SELECT razon_social, date_format(fservicio, '%d/%m/%Y'),
                   ...
                   o.nombre as nomord, ot.capacidad_solicitada, oxf.id as fact, ot.id as ordturismo, datediff(date(now()), fservicio) as dias,
                   interno, date_format(o.hsalida, '%H:%i') as hsalida
     ...
     while ($data){
           ...
           while (($data)&&($cliente == $data['0'])){
                 ...
                 $tabla.="<tr bgcolor='$color'>
                              <td align='left'><a href='../../vista/ordenes/ordtur.php?otmo=$data[7]&fd=$desde&fh=$hasta'>$data[0]</a></td>
                              <td align='center'>$data[1]</td>
                              <td align='center'>$data[hsalida]</td>
                              <td align='left'>".htmlentities($data[6])."</td>
                              <td>".htmlentities($data[nomord])."</td>
                              <td>$data[interno]</td>
                              <td>$data[capacidad_solicitada]</td>
