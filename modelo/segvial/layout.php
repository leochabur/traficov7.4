<?php
  ...
  if ($accion == 'list'){ ///codigo para guardar ////
     $conn = conexcion(true);
     $sql = "SELECT interno, nueva_patente, patente, upper(e.razon_social) as propietario, 
             ...
             LEFT JOIN layout_unidades lu on lu.id_unidad = u.id
             WHERE e.activo and tu.id <> 8 and e.id = $_POST[propietario] AND u.activo
             ORDER BY e.razon_social, interno";
    ...
    while($data = mysqli_fetch_array($result))
    {
        ...
        $uploadInf = '<form class="fupFormInf" enctype="multipart/form-data">
                            <input type="file" class="form-control" id="file" name="file" required />
                            <input type="hidden" id="accion" name="accion" value="laysup"/>
                            <input type="hidden" id="unidad" name="unidad" value="'.$data['unidad'].'"/>
                            <input type="hidden" id="location" name="location" value="INF"/>
                        <input type="submit" name="submit" class="btn btn-success submitBtnInf" value="Enviar"/>
                    </form>';
        $uploadSup = '';
        $us = '';
        $ui = '<i class="far fa-times-circle fa-2x"></i>';
        if ($data['url_inferior'])
        {
          $ui = '<a target="_blank" href="'.$uploadDir.'/'.$data['url_inferior'].'"><i class="fas fa-eye fa-2x"></i></a>';
        }
        if ($data['idtipo'] == 1)
        {
          $uploadSup = '<form class="fupFormInf" enctype="multipart/form-data">
                            <input type="file" class="form-control" id="file" name="file" required />
                            <input type="hidden" id="accion" name="accion" value="laysup"/>
                            <input type="hidden" id="unidad" name="unidad" value="'.$data['unidad'].'"/>
                            <input type="hidden" id="location" name="location" value="SUP"/>
                        <input type="submit" name="submit" class="btn btn-success submitBtnInf" value="Enviar"/>
                    </form>';
          $us = '<i class="far fa-times-circle fa-2x"></i>';
          if ($data['url_superior'])
          {
            $us = '<a target="_blank" href="'.$uploadDir.'/'.$data['url_superior'].'"><i class="fas fa-eye fa-2x"></i></a>';
          }
        }
    ...
    $script = "<script>
                    $('.fupFormInf').on('submit', function(e){                              
                              e.preventDefault();
                              var f = $(this);
                              f.find('.submitBtnInf').attr('disabled','disabled');
                              f.css('opacity','.5');
                              $.ajax({
                                        type: 'POST',
                                        url: '/modelo/segvial/layout.php',
                                        ...
                                        processData:false                                        
                                                                  ...
                                                                  {
                                                                    f[0].reset();
                                                                    var ch = f.closest('tr').children('.'+response.location);
                                                                    ch.html(\"<a target='_blank' href='\"+response.path+\"'><i class='fas fa-eye fa-2x'></i></a>\");
                                                                  }                                                                 
                                                                  f.css('opacity','');
                                                                  f.find('.submitBtnInf').removeAttr('disabled');                                                                      
