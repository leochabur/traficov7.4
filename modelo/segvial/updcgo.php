<?
  ...
  elseif ($accion == 'res'){
     ...
     try{
         ...
         $campos = "usr_alta, fecha_alta, fecha_respuesta, resolucion, detalle_resolucion";
         $values = "$_SESSION[userid], now(), '$fecha', $_POST[res], '".str_replace(",",";",$_POST[det])."'";
         begin($conn);
         backup("descargos", "descargos_modificados", "nro_descargo = $_POST[num]", $conn);
         update("descargos", "$campos", $values, "nro_descargo = $_POST[num]", $conn);
         commit($conn);
         $response[msge] = "Se ha almacenado con exito la resolucion en la BD";
         cerrarconexcion($conn);
         print json_encode($response);
     }
     catch (Exception $e) {
                           $response[status] = false;
                           $response[msge] = "Se ha producido un error al intentar guardar la resolucion!!!";
                           $response[sql] = $e->getMessage();
                           cerrarconexcion($conn);
                           print json_encode($response);
                          };
