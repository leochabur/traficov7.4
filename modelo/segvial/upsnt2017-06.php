          ...
          if ($_POST[acccor]){   //selecciono una fecha de accion correctiva
             if ($_POST[feacc]){ //si selecciono una fecha de accion correctiva debe registrarse la fecha de la misma
                ...
                if ($row = mysql_fetch_array($result)){
                   if ($row[0]){
                      $sql="select upper(apenom)
                            from usuarios u
                            where u.id = $_POST[solacccor]";//obtiene el sector del usuario logeado
                      $result=mysql_query($sql, $conn);
                      $convocante = "";
                      if ($row = mysql_fetch_array($result)){
                         $convocante=$row[0];
                      ...
                      $campos_minuta="id, id_siniestro, id_empleado, id_responsable, sector_convocante, fecha, temas_a_tratar, medidas_adoptadas, fecha_alta, id_user, fecha_implementacion, fecha_verificacion";
                      $valores_minuta="$_POST[nsin], $_POST[condsin], $_POST[solacccor], '$convocante', '$fechaacc', '$detalle', '".str_replace(",",";",$_POST[detresin])."', now(), $_SESSION[userid], $impl, $veri";
                      $mta=insert('minuta_de_reunion', $campos_minuta, $valores_minuta, $conn);
                      $response[mtasn]=true;
                      $response[mtanro]=$mta;
                   }
                }
