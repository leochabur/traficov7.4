<?
  ...
  elseif($accion == 'load'){
          $conn = conexcion();
          $sql = "SELECT u.id as id_unidad, interno, patente, marca, modelo, marca_motor, anio, cantasientos, if(u.activo, 'checked', '') as activo, if(video, 'checked', '') as video, if(bar, 'checked', '') as bar, if(banio, 'checked', '') as banio, id_calidadcoche, id_tipounidad, consumo, id_propietario, calidad,  tipo, e.id as id_propietario, e.razon_social
                  FROM (SELECT * FROM unidades WHERE (id = $_POST[unidad]) and (id_estructura = $_SESSION[structure])) u
                ...
                <form id="modunidad">
                         <table border="0" align="center" width="50%" name="tabla">
                                ...
                                <tr>
                                    ...
                                    <td><input id="dominio" name="dominio" size="8" value="'.$data['patente'].'" class="required ui-widget ui-widget-content  ui-corner-all" minlength="2"/></td>
                                    <td></td>
                                </tr>
                                <tr>
                                ...
                                <td WIDTH="20%">Tipo Unidad</td>
                                    ...
                                    '<option value="'.$data['id_propietario'].'">'.$data['tipo'].'</option>'.
  ...
  elseif($accion == 'upduda'){
          ...
          $campos = "marca_motor, anio, patente, marca, modelo, cantasientos, video, bar, banio, activo, consumo, id_propietario, id_estructura_propietario";
          $values = "'$_POST[motor]', '$_POST[anio]', '$_POST[dominio]', '$_POST[marca]', '$_POST[modelo]', '$_POST[cantas]', $video, $bar, $banio, $activo, '$_POST[consumo]', $_POST[propietario], $_SESSION[structure]";
  ...
  elseif($accion == 'list'){
          $sql="SELECT upper(e.nombre) as estr, u.id as id_unidad, interno, patente, marca, modelo, marca_motor, anio, cantasientos, if(u.activo, 'checked', '') as activo, if(video, 'checked', '') as video, if(bar, 'checked', '') as bar, if(banio, 'checked', '') as banio, id_calidadcoche, id_tipounidad, consumo, calidad,  tipo
                FROM (SELECT * FROM unidades WHERE (id_propietario = $_POST[propietario]) and (id_estructura_propietario = $_SESSION[structure])) u
                LEFT JOIN calidadcoche cc ON (cc.id = u.id_calidadcoche) and (cc.id_estructura = u.id_estructura_calidadcoche)
                LEFT JOIN tipounidad tu ON (tu.id = u.id_tipounidad) and (tu.id_estructura = u.id_estructura_tipounidad)
          ...
          while($data = mysql_fetch_array($result)){
                      $tabla.="<tr>
                                   <td><a href='../../vista/segvial/moduda.php?int=$data[id_unidad]'>$data[interno]</a></td>
                                   <td>$data[patente]</td>
                                   <td>$data[marca]</td>
                                   <td>$data[modelo]</td>
