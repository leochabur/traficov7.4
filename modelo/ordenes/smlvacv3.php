...
elseif ($accion == 'renord')
{
   ...
   $sql="select o.id as id, o.nombre as nombre, id_servicio
                ...
                where o.id_estructura = 1 and fservicio = '$fecha' and
                      id_TipoServicio = 2 and not borrada and not suspendida and c.id_cliente = 10
                order by id_servicio, nombre";
  
   $result = mysqli_query($conn, $sql);
   $table = "<table border='1' class='table table-zebra'>
              <thead>
                <tr>
   ...
   $indice = array(0=> '- (A)', 1 => '- (B)', 2 => '- (C)', 3 => '- (D)', 4 => '- (E)', 5 => '-(F)');
   while ($data)
   {
...
function getKmTiempo($conn, $origen, $destino, $str, $latiO, $longO, $latiD, $longD, $log, $numOrden = ""){    ///recupera la distancia y el tiempo de viaje desde el origen al destino
         ...
         if ($row = mysqli_fetch_array($result)){
          ...
          //    die("$origen, $destino, $str, $latiO, $longO, $latiD, $longD");
           ...
           //   if (!isset($km) || !isset($tiempo)){
                 ...
                 $result = mysqli_query($conn, $sql)or die(mysqli_error($conn)."  - log: $log  origen: $origen   _   destino:  $destino   _  orden: $numOrden -".$sql);
                         ...
                         {                            
                              ...
                              if (in_array($ultorden['id_chofer_1'], $conductores)){ ///el conductor se queda con el coche  ##A##                              
                                  if ($ultorden['idCityC1'] == $ultorden['destino']){ ///termino el servicio donde vive el conductor ##B##
                                      ...
                                      $totalKmLlendoALaCasa = $kmtpoDestResidencia[0]+$kmtpoResidenciaOrigen[0];
                                      if (($second - $tiempoViajeLlendoALaCasa) > $amplitudCondFijo){ ///el tiempo le da para ir al lugar de residencia del conductor??????
                                          ...
                                          if (in_array($row['id'], $ordenesProcesadas)) // ya ha procesado la orden, solo debe actualizar el segundo condutor de la orden
                                          {
                                              ...
                                              createOrden($str, $citaVacio->format('Y-m-d'), $nombre,
                                                  ...
                                                  $kmtpoDestResidencia[0], $ultorden['destino'], $ultorden['idCityC1'], $cliVac[0], $ultorden['id_cliente'],
                                  ...
                                  if (($ultorden['destino'] == $cabecera[0])){//termino en la cabecera
                                      ...
                                      if ($row['origen'] != $cabecera[0]){
                                          ...
                                          if (in_array($row['id'], $ordenesProcesadas)){ // ya ha procesado la orden, solo debe actualizar el segundo condutor de la orden                                     
                                             ...
                                             createOrden($str, $saleVacio->format('Y-m-d'), $nombre,
                                                 ...
                                                 $kmtpoCabeceraOrigen[0], $cabecera[0], $row['origen'], $cliVac[0], $row['id_cliente'],
                                      ...
                                      $kmViajeLlendoACabecera = $kmtpoDestinoCabecera[0]+$kmtpoCabeceraOrigen[0];
                                      
                                     
                                      if (($second - $tiempoViajeLlendoACabecera) > $amplitud){///tiene tiempo de ir a la cabecera
                                          ...
                                          if (in_array($row['id'], $ordenesProcesadas)){ // ya ha procesado la orden, solo debe actualizar el segundo condutor de la orden
                                              ...
                                              $tabla.= imprimir($citaVacio, $citaVacio, $llegaVacio, $llegaVacio, $nombre, $conductor, $ultorden['id_cliente'], $clientes, $color, 564, $cliVac[0]);
                                          }
                                          else{                                                                                                          
                                              ...
                                              createOrden($str, $citaVacio->format('Y-m-d'), $nombre,
                                                  $citaVacio->format('H:i:s'), $citaVacio->format('H:i:s'), $llegaVacio->format('H:i:s'), $llegaVacio->format('H:i:s'),
                                                  $kmtpoDestinoCabecera[0], $ultorden['destino'], $cabecera[0], $cliVac[0], $ultorden['id_cliente'],
                                                  $ultorden['id_chofer_1'], $ultorden['id_chofer_1'], $ultorden['id_micro'], $ultorden['id'], $conn, $simular, $id_Diag_Vacio, $conds, 1);
                                              $tabla.= imprimir($citaVacio, $citaVacio, $llegaVacio, $llegaVacio, $nombre, $conductor, $cliVac[0], $clientes, $color, 579, $cliVac[0]);
                                              
                                             // throw new Exception("  SECOND: $generarOrdenCabeceraAOrigen");
                                          }
                                          if ($row['origen'] != $cabecera[0]){
                              ...
                              if (($generarOrdenDestinoAOrigen) && ($ultorden['destino'] != $row['origen'])){///debe generar una orden desde el destino ultimo al origen actual                                  
                                  ...
                                  $saleVacio = new DateTime($ultorden['hfina']);
                              ...
                              if (($generarOrdenCabeceraAOrigen) && ($cabecera[0] != $row['origen'])){///debe generar una orden desde la cabecera al origen actual
                                  $kmtpo = getKmTiempo($conn, $cabecera[0], $row['origen'], $str, $cabecera[1], $cabecera[2], $row['latiO'], $row['longO'],9);
                                  if (!($kmtpo[1] >= 0)  ||  !($kmtpo[0] >= 0))
                                  {
                                      throw new Exception("CE-2.0 No se pudo calcular la ruta para el recorrido $row[nombre] (".$clientes[$row['id_cliente']].") - ($cabecera[3] - $row[cityO]) - Verifique que las dos ciudades tengan cargada su Latitud y su Longitud");
                                  }
                                  $llegaVacio = new DateTime($row['hcita']);
                                  $saleVacio = clone $llegaVacio;
                                  
                                  try{
                                      $ultimo = new DateTime($ultorden['hfina']);
                                      $saleVacio->sub(new DateInterval("PT$kmtpo[1]M"));
                                      $citaVacio = clone $saleVacio;
                                      $corte = ($llegaVacio->format('U') - $ultimo->format('U'));
                                      $corte = ($corte /60);
                                      if ($corte >= $minCorte){
                                          $citaVacio->sub(new DateInterval("PT30M"));
                                      }
                                  } catch (Exception $e){throw new Exception("ERROR LINEA 600");}
                                  ...
                                  else
                                  {
                                     // $kmtpo = getKmTiempo($conn, $ultorden['destino'], $row['origen'], $str, $ultorden['latiD'], $ultorden['longD'], $row['latiO'], $row['latiO'],6);
                                      ...
                                      createOrden($str, $citaVacio->format('Y-m-d'), $nombre,
                                          $citaVacio->format('H:i:s'), $saleVacio->format('H:i:s'), $llegaVacio->format('H:i:s'), $llegaVacio->format('H:i:s'),
                                          $kmtpo[0], $ultorden['destino'], $row['origen'], $cliVac[0], $row['id_cliente'],
                                          $row['id_chofer_1'], $row['id_chofer_1'], $row['id_micro'], $row['id'], $conn, $simular, $id_Diag_Vacio, $conds, 3);
                                  }
                                  $tabla.= imprimir($citaVacio, $saleVacio, $llegaVacio, $llegaVacio, $nombre, $conductor, $row['id_cliente'], $clientes, $color, 614, $cliVac[0]);
                              }
                        ...
                        else ///la orden aun no ha sido procesada  (ELSE)
                        {
                            ...
                            else
                            {
                                 if ($ultorden['destino'] != $ultorden['idCityC1']){
                                    $nombre = "VACIO ($ultorden[cityD] - $ultorden[cityC1])~(".$clientes[$ultorden['id_cliente']].")";
                                    $kmtpo = getKmTiempo($conn, $ultorden['destino'], $ultorden['idCityC1'], $str, $ultorden['latiD'], $ultorden['longD'], $row['latCico1'], $row['longCico1'],11, "$ultorden[id]  -  $row[id]");
                                    if (!($kmtpo[1] >= 0)  ||  !($kmtpo[0] >= 0)){
