...
try{
    ...
    $tabla = "<style>
         ...
         while ($row){
                         ...
                         {                            
                              ...
                              if (in_array($ultorden['id_chofer_1'], $conductores)){ ///el conductor se queda con el coche  ##A##                              
                                  if ($ultorden['idCityC1'] == $ultorden['destino']){ ///termino el servicio donde vive el conductor ##B##
                                      ...
                                      $kmtpoDestResidencia = getKmTiempo($conn, $ultorden['destino'], $ultorden['idCityC1'], $str, $ultorden['latiD'], $ultorden['longD'], $ultorden['latCico1'], $ultorden['longCico1']); 
                                      ...
                                      $kmtpoResidenciaOrigen = getKmTiempo($conn, $row['idCityC1'], $row['origen'], $str, $row['latCico1'], $row['longCico1'], $row['latiO'], $row['longO']);//calcula la distancia desde la cabecera a la salida del recorrido
                                  ...
                                  //SI   
                                      if (in_array($row['id'], $ordenesProcesadas)){ // ya ha procesado la orden, solo debe actualizar el segundo condutor de la orden                                     
                                          ...
                                          $tabla.= imprimir($citaVacio, $citaVacio, $citaVacio, $llegaVacio, $nombre, $conductor, $row['id_cliente'], $clientes, $color);                                          
                                     }
                                     else{
                                         ...
                                         $kmtpoCabeceraOrigen = getKmTiempo($conn, $cabecera[0], $row['origen'], $str, $cabecera[1], $cabecera[2], $row['latiO'], $row['longO']);
                                      ...
                                      $kmtpoDestinoCabecera = getKmTiempo($conn, $ultorden['destino'], $cabecera[0], $str, $ultorden['latiD'], $ultorden['longD'], $cabecera[1], $cabecera[2]); 
              ...
              /*-IF 15-*/     if ($ultorden['destino'] != $row['origen']) ///la ultima orden finalizo en un lugar distinto al de donde sale la orden actual    
							  {                   
            ...
            /*-FIN ELSE 15-*/ }
                              ...
                              $tabla.= imprimir($hcita, $hcita, $hcita, $hfina, $row['nombre'], $conductor, $row['id_cliente'], $clientes, $color);
						               ...
						               {                                
                                ///$cabecera  array(id, lati, long, ciudad) 
                                //getKmTiempo($conn, $origen, $destino, $str, $latiO, $longO, $latiD, $longD)
                                //imprimirOrden($fecha, $hcita, $hsale, $nombre, $cliente, $hllega, $hfina);
                                ///esta procesadno la primer orden del servicio, lo primero que debe verificar es si ya proceso la orden, 
                                ///quiere decir que la orden del vacio tiene un segurno conductor, debe asignarle el segundo conductor al vacio correspondiente
                                if (in_array($row['id'], $ordenesProcesadas)) // ya ha procesado la orden, solo debe actualizar el segundo condutor de la orden
                                {
                                    actuaizarConductoresOrdenVacio($conn, $row['id'], $row['id_chofer_1'], $str);
                                }
                                else ///la orden aun no ha sido procesada  (ELSE 5)
                                {
                                    //$ordenesProcesadas[] = $row['id']; ///almacena la orden como procesada
                                    ...
                                    if ($row['origen'] != $cabecera[0]) ///el servicio no sale de la cabecera
                                    {
                                        ...
                                        {   
                                            ...
                                            if ($row['idCityC1'] != $row['origen'])
                                            {
                                                ...
                                                $kmtpo = getKmTiempo($conn, $row['idCityC1'], $row['origen'], $str, $row['latCico1'], $row['longCico1'], $row['latiO'], $row['longO']);
                                                if (!($kmtpo[1] >= 0)  ||  !($kmtpo[0] >= 0))
                                                ...
                                                $saleVacio->sub(new DateInterval("PT$kmtpo[1]M"));
                                                                       ...
                                                                       {  
                                                                          throw new Exception("ERROR LINEA 573");
                                                                       }
                                                $nombre = "VACIO ($row[cityC1] - $row[cityO])~(".$clientes[$row['id_cliente']].")";                                                
                                                createOrden($str, $citaVacio->format('Y-m-d'), $nombre,
                                                ...
                                                $tabla.= imprimirOrden($citaVacio, $citaVacio, $saleVacio, $nombre, $clientes[$cliVac[0]], $hcita, $hcita);
                                              }
                                        }
                                        else  //el conducotr no se queda con el coche, debe generar un vacio desde la cabecera al lugar de salida
                                        {
                                            //calcula los tiempos y km desde la cabecera al lugar de salida del servicio
                                            $kmtpo = getKmTiempo($conn, $cabecera[0], $row['origen'], $str, $cabecera[1], $cabecera[2], $row['latiO'], $row['longO']);
                                            if (!($kmtpo[1] >= 0)  ||  !($kmtpo[0] >= 0))
                                            {
                                               throw new Exception("CE-2.0 No se pudo calcular la ruta para el recorrido $row[nombre] (".$clientes[$row['id_cliente']].") - ($cabecera[3] - $row[cityO]) - Verifique que las dos ciudades tengan cargada su Latitud y su Longitud");
                                            }
                                ...
                                $tabla.= imprimirOrden($hcita, $hcita, $hsale, $row['nombre'], $clientes[$row['id_cliente']], $hllega, $hfina);
                           }
                           $ultorden = $row;
                           $row = mysqli_fetch_array($result);
                     ...
                     if ($ultorden['destino'] != $cabecera[0]) ////si el servicio no termina en la cabecera debe evaluar a donde genera el vacio
                       ...
                       // if (in_array($row['id_chofer_1'], $conductores)) ///el conductor se queda con el coche
                        $kmtpo = getKmTiempo($conn, $cabecera[0], $ultorden['destino'], $str, $cabecera[1], $cabecera[2], $ultorden['latiD'], $ultorden['longD']);
                        if (!($kmtpo[1] >= 0)  ||  !($kmtpo[0] >= 0)){
                      ...
                      /*  if ($hfina->format('H') == 0){
                        ...
                        else ///la orden aun no ha sido procesada  (ELSE)
                        {
                            ...
                            if (!in_array($ultorden['id_chofer_1'], $conductores))  
                            {
                               ...
                               $tabla.= imprimir($hfina, $hfina, $hfina, $hfinaVacio, $nombre." - CREATE", $conductor, $cliVac[0], $clientes, $color);
                            }
                            else
                            {
                                 if ($ultorden['destino'] != $ultorden['idCityC1']){
                                    $nombre = "VACIO ($ultorden[cityD] - $ultorden[cityC1])~(".$clientes[$ultorden['id_cliente']].")";
                                    $kmtpo = getKmTiempo($conn, $ultorden['destino'], $row['idCityC1'], $str, $ultorden['latiD'], $ultorden['longD'], $row['latCico1'], $row['longCico1']);
                                    if (!($kmtpo[1] >= 0)  ||  !($kmtpo[0] >= 0)){
...
function getKmInterCabecera($conn, $cabecera, $row, $clientes, $ultorden, $str){
    $kmcabor = getKmTiempo($conn, $cabecera[0], $row['origen'], $str, $cabecera[1], $cabecera[2], $row['latiO'], $row['longO']);
    //calcula la distancia desde la cabecera a la salida del recorrido
    if (!($kmcabor[1] >= 0)  ||  !($kmcabor[0] >= 0)){
        throw new Exception("CE-5.0 / No se pudo calcular la ruta para el recorrido $row[nombre] (".$clientes[$row['id_cliente']].") - ($cabecera[3] - $row[cityO]) - Verifique que las dos ciudades tengan cargada su Latitud y su Longitud");
    }
    
    $kmdestcab = getKmTiempo($conn, $ultorden['destino'], $cabecera[0], $str, $ultorden['latiD'], $ultorden['longD'], $cabecera[1], $cabecera[2]);
    ///calcula distancia del ultimo destino a la cabecera
