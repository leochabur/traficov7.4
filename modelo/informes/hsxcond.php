...
$sql = "select o.*, concat(apellido,', ', nombre) as emple, legajo, concat(fservicio_inicio,' ',hcitacion) as dtdesde, concat(fservicio_fin,' ',hfinservicio) as dthasta
            from(
            SELECT id_chofer_1 as cond, hcitacion, hsalida, hllegada, hfinservicio,
                   ...
                   id_ciudad_origen as origen, id_ciudad_destino as destino
FROM ordenes o
...
$conn = conexcion();
     ...
     while ($data){
           ...
           while (($data) &&($cond == $data['cond'])){
                 ...
                 if ($ultima_vuelta){
                    if (datetomin($ultima_vuelta->diff($desde))>= 720)
                          $reiniciar=false;//die ($desde->format('Y-m-d H:i:s')."   paso por ult vueltassa s ".$hasta->format('Y-m-d H:i:s')." ".$minutos_al_100);
                 ...
                 if (!($reiniciar)){
                     ...
                     if (datetomin($ultima_vuelta->diff($desde))>= 720){
                        $descansa12=1;
                        $reiniciar = true;
                        $total_min_normales+=$minutos_acumulados_dia;
                        $minutos_acumulados_dia=0;
                        $total_min_50+=$minutos_al_50;
                        $minutos_al_50=0;
                        $total_min_100+=$minutos_al_100;
                        $minutos_al_100=0;
                        $corte=0;
                        $ultima_vuelta=0;
                        $acata=1;
                     ...
                     if ($desde >=$corte100){
                        ...
                        $ultima_vuelta=0;
                          ...
                          if ($hasta >= $corte100){
                             if ($descansa12){
                             $desde = new DateTime($corte100->format('Y-m-d H:i:s'));
                             }
                             else
                                 $total_min_100+=datetomin($desde->diff($corte100));
                             $reiniciar = true;
                             $total_min_normales+=$minutos_acumulados_dia;
                             $minutos_acumulados_dia=0;
                             $total_min_50+=$minutos_al_50;
                 ...
                 elseif (($desde <= $corte50) && ($hasta > $corte50)){
                      $auxii= $minutos_al_50;
                       ...
                       //if (datetomin($desde->diff($hasta))

                       $minutos_al_100+= datetomin($corte50->diff($hasta));  //diferencia entre el corte al 50 y el fin de orden para calcular horas al 100
              //         if ($minutos_al_100>0)
                //          die ($ultima_vuelta->format('Y-m-d H:i:s')."   xvsfsfsf".$desde->format('Y-m-d H:i:s')." ".$minutos_al_100);
                       $i++;
                 }
                 elseif ($desde > $corte50){
                        $minutos_al_100+= datetomin($desde->diff($hasta))+$minutos_corte;
            //            if ($minutos_al_100>0)
              //            die ($ultima_vuelta->format('Y-m-d H:i:s')."  ddddddd ".$desde->format('Y-m-d H:i:s')." ".$minutos_al_100);
                 }

                     //       if ($minutos_al_100 >0)
                       //   die ($ultima_vuelta->format('Y-m-d H:i:s')."  genererarar ".$desde->format('Y-m-d H:i:s')." ".$minutos_al_100);
                 $ultima_vuelta = new DateTime($hasta->format('Y-m-d H:i:s'));
                 $ultimo_destino = $data['destino'];
                 $data = mysql_fetch_array($result);
           }

           //actualiza los acumuladores porque sino pierde los datos de la ultima orden procesada
           $total_min_normales+=$minutos_acumulados_dia;
           $minutos_acumulados_dia=0;
           $total_min_50+=$minutos_al_50;
           $minutos_al_50=0;
        ...
        //   die($aux);
           die("minutos acumulados dia: ".($total_min_normales)."    minutos al 50: ".($total_min_50)."            minutos al 100: ".($total_min_100));
     }
     $tabla.='</tbody>
