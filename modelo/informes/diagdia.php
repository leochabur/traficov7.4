<?php
  ...
  if (true)
  {
     ...
     {
        $excluir1 = $excluir2 = $excluir3 = '';
     }
     else
     {
        $excluir2 = 'where ch2.id_cargo in (1,2,3,4)';
        $excluir3 = 'where em.id_cargo in (1,2,3,4)';
        $excluir1 = 'where ch1.id_cargo in (1,2,3,4)';
     }

    $sql="SELECT o.*, if (edd.id is not null, hcitacionreal, hcitacion) as orden
          FROM(	(SELECT interno, upper(c.razon_social)as razon_social, 0 as id_cond, fservicio as fsrv, upper(servicio) as servicio, 
                 ...
                 FROM (SELECT  if (ot.id is null, 0, ot.id) as id_turismo, nombre as servicio, hsalida, hfinservicio, ord.id, id_chofer_1, fservicio, hcitacion, id_cliente, 
      ...
      upper(CONCAT(nov_text, ' (',date_format(desde, '%d/%m/%Y'), ' - ', date_format(hasta, '%d/%m/%Y'),')')) as servicio,
        ...
        LEFT JOIN (SELECT id, id_estructura, fecha FROM estadoDiagramasDiarios WHERE id_estructura = $_SESSION[structure] AND finalizado = 1) edd ON edd.fecha = o.fsrv
        $cond
        order by apellido, id_cond, li, orden";
