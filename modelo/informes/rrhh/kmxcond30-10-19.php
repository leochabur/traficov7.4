<?php
  ...
  if ($accion == 'list'){
    ...
    $sql="SELECT o.*
          FROM(
                (SELECT interno, upper(c.razon_social)as razon_social, 0 as id_cond, fservicio as fsrv, upper(servicio) as servicio, date_format(hsalida, '%H:%i') as hsalida, 
                       o.id, date_format(fservicio, '%d/%m/%Y') as fservicio, date_format(hcitacion, '%H:%i') as hcitacion, '                  ORDENES A DESIGNAR' as apellido, 
                       0 as legajo, fservicio as li, fservicio as ls, DATEDIFF(fservicio, fservicio) as dias, date_format(hfinservicio, '%H:%i') as hfinservicio, comentario, 
                 ...
                 FROM (SELECT  nombre as servicio, hsalida, hfinservicio, id, id_chofer_1, fservicio, hcitacion, id_cliente, 
                             id_estructura_cliente, id_micro, comentario, hllegadaplantareal, hsalidaplantareal, hfinservicioreal, hcitacionreal
                       FROM (SELECT * FROM ordenes WHERE id_estructura = $_SESSION[structure]) o
                       WHERE ((fservicio between '$desde' and '$hasta') and (id_estructura = $_SESSION[structure])) and (not suspendida) and (not borrada) and (id_chofer_1 is null) and (id_chofer_2 is null)) o
               ...
               UNION
			         (SELECT interno, upper(c.razon_social)as razon_social, o.id_chofer_1 as id_cond, fservicio as fsrv, upper(servicio) as servicio, date_format(hsalida, '%H:%i') as hsalida, 
                       ...
                       date_format(hfinservicio, '%H:%i') as hfinservicio, comentario, hllegadaplantareal, hsalidaplantareal, hfinservicioreal, hcitacionreal
                FROM (SELECT  nombre as servicio, hsalida, hfinservicio, id, id_chofer_1, fservicio, hcitacion, id_cliente, 
                              id_estructura_cliente, id_micro, comentario, hllegadaplantareal, hsalidaplantareal, hfinservicioreal, hcitacionreal
			                FROM (SELECT * FROM ordenes WHERE id_estructura = $_SESSION[structure]) o
			          WHERE ((fservicio between '$desde' and '$hasta') and (id_estructura = $_SESSION[structure])) and (not suspendida) and (not borrada)) o
               ...
               UNION ALL
        			(SELECT '' as interno, '' as razon_social, n.id_empleado as id_cond, '$desde' as fsrv, upper(CONCAT(nov_text, ' (',date_format(desde, '%d/%m/%Y'), ' - ', date_format(hasta, '%d/%m/%Y'),')')) as servicio, '00:00' as hsalida, 
                      ...
                      '' as comentario, '00:00' as hllegadaplantareal, '00:00' as hsalidaplantareal, '00:00' as hfinservicioreal, '00:00' as hcitacionreal
