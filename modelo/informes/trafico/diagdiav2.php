<?php
  ...
  if (true){
    ...
    $sqlNovedades = "SELECT n.id_empleado, upper(concat(if (emp.id = 1, '', concat('(', upper(emp.razon_social),') - ')), apellido,', ', nombre)) as cond, legajo, 
                            ...
                            upper(CONCAT(nov_text, ' (',date_format(desde, '%d/%m/%Y'), ' - ', date_format(hasta, '%d/%m/%Y'),')')) as descripcion, legajo, cn.id as idNovedad
                     FROM novedades n
      ...
      {
          $novedades[$row['id_empleado']] = $row['idNovedad'];
...
//die($sqlOrdenes);
      ...
      if (!$cond)
      {
              $sqlOrdenesSinConductor = "SELECT fservicio, citaReal, saleReal, llegadaReal, hfinservicioReal, hcitacion, hsalida, hllegada, hfinservicio, upper(ordenes.nombre) as nombre, 
                                    ' A DESIGNAR' as cond, 
                                    0 as idEmp, upper(c.razon_social) as razon_social, interno, idOrden, ot.id as id_turismo
                             FROM
                             (   
                                SELECT *
                                FROM (SELECT fservicio, nombre,
        ...
        foreach ($empleados as $k => $cond)
        {
          ...
          {   
             ...
             foreach ($ordenes as $ord)
             {
                   ...
                   if ($ord['ot'])
                   {
                    $tsmo.='<img class="turismo" src="../../../vista/maleta.png" width="20" height="20" border="0">';
                   }
                   $tabla.="<tr bgcolor='$color' >
                                <td width='10%' align='center' data-id='".$ord['id']."'>".$ord['fec']."</td>
                                <td width='7%' align='center' data-id='".$ord['id']."'>".$ord['hc']."</td>
                                <td width='7%' align='center' data-id='".$ord['id']."'>".$ord['hsr']."</td>
                                <td width='25%' data-id='".$ord['id']."'>".$ord['desc']."</td>
                                <td width='5%' data-id='".$ord['id']."'>".$ord['int']."</td>
                                <td width='10%' data-id='".$ord['id']."'>".$ord['cli']."</td>
                                <td width='10%' align='center' data-id='".$ord['id']."'>".$ord['hfs']."</td>
                                <td width='10%' align='center' data-id='".$ord['id']."'>".$ord['hfr']."</td>                    
                                <td style='width:2%;' class='turismo' data-tmo='$ord[ot]' data-id='".$ord['id']."'>$tsmo</td>
                            </tr>";
             }
             $j++;
          }
