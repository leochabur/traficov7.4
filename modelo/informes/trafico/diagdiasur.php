<?php
  ...
  if (true){
    ...
    $sql="SELECT o.*
          FROM(	(SELECT interno, upper(c.razon_social)as razon_social, 0 as id_cond, fservicio as fsrv, upper(servicio) as servicio, date_format(salida, '%d/%m/%Y %H:%i') as hsalida, o.id, date_format(fservicio, '%d/%m/%Y') as fservicio, date_format(citacion, '%d/%m/%Y %H:%i') as hcitacion, '                  ORDENES A DESIGNAR' as apellido, 0 as legajo, fservicio as li, fservicio as ls, DATEDIFF(fservicio, fservicio) as dias, date_format(finalizacion, '%d/%m/%Y %H:%i') as hfinservicio, emp.color, date_format(llegada, '%d/%m/%Y %H:%i') as hllegada, id_turismo, 0 as id_empleador
                 FROM (SELECT  if (ot.id_viaje is null, 0, ot.id_viaje) as id_turismo, nombre as servicio, salida, finalizacion, ord.id, id_chofer_1, date(citacion) as fservicio,  citacion, id_cliente, id_estructura_cliente, id_micro, llegada
                       FROM ordenes ord
                ...
                UNION
			(SELECT interno, upper(c.razon_social)as razon_social, o.id_chofer_1 as id_cond, fservicio as fsrv, upper(servicio) as servicio, date_format(salida, '%d/%m/%Y %H:%i') as hsalida, o.id, date_format(fservicio, '%d/%m/%Y') as fservicio, date_format(citacion, '%d/%m/%Y %H:%i') as hcitacion, upper(if(emp.id = 1,concat(ch1.apellido, ', ',ch1.nombre), concat('(',emp.razon_social,') ', ch1.apellido, ', ',ch1.nombre))) as apellido, concat(legajo, emp.id) as legajo, date(citacion) as li, fservicio as ls, DATEDIFF(fservicio, fservicio) as dias, date_format(finalizacion, '%d/%m/%Y %H:%i') as hfinservicio, empl.color, date_format(llegada, '%d/%m/%Y %H:%i') as hllegada, id_turismo, ch1.id_empleador
       FROM (SELECT  if (ot.id_viaje is null, 0, ot.id_viaje) as id_turismo, nombre as servicio, salida, finalizacion, ord.id, id_chofer_1, date(citacion) as fservicio, citacion, id_cliente, id_estructura_cliente, id_micro, llegada
             FROM ordenes ord
             LEFT JOIN tur_ordenes_por_viaje ot on ot.id_orden = ord.id 
             INNER JOIN horarios_ordenes_sur hhs ON hhs.id_orden = ord.id AND hhs.id_estructura_orden = ord.id_estructura
        ...
        UNION
			(SELECT interno, upper(c.razon_social)as razon_social, o.id_chofer_2 as id_cond, fservicio as fsrv, upper(servicio) as servicio, date_format(salida, '%d/%m/%Y %H:%i') as hsalida, o.id, date_format(fservicio, '%d/%m/%Y') as fservicio, date_format(citacion, '%d/%m/%Y %H:%i') as hcitacion, upper(if(emp.id = 1,concat(ch2.apellido, ', ',ch2.nombre), concat('(',emp.razon_social,') ', ch2.apellido, ', ',ch2.nombre))) as apellido, concat(legajo, emp.id) as legajo, date(citacion) as li, fservicio as ls, DATEDIFF(fservicio, fservicio) as dias, date_format(finalizacion, '%d/%m/%Y %H:%i') as hfinservicio, empl.color, date_format(llegada, '%d/%m/%Y %H:%i') as hllegada, id_turismo, ch2.id_empleador
             FROM (SELECT  if (ot.id_viaje is null, 0, ot.id_viaje) as id_turismo, nombre as servicio,finalizacion, salida, ord.id, id_chofer_2, date(citacion) as fservicio, citacion, id_cliente, id_estructura_cliente, id_micro, llegada
                   FROM ordenes ord
                   LEFT JOIN tur_ordenes_por_viaje ot on ot.id_orden = ord.id
                   INNER JOIN horarios_ordenes_sur hhs ON hhs.id_orden = ord.id AND hhs.id_estructura_orden = ord.id_estructura
                   ...
                   UNION ALL
                   SELECT  if (ot.id_viaje is null, 0, ot.id_viaje) as id_turismo, nombre as servicio,finalizacion, salida, ord.id, id_empleado, date(citacion) as fservicio, citacion, id_cliente, id_estructura_cliente, id_micro, llegada
                   FROM ordenes ord
                   LEFT JOIN tur_ordenes_por_viaje ot on ot.id_orden = ord.id
                   INNER JOIN horarios_ordenes_sur hhs ON hhs.id_orden = ord.id AND hhs.id_estructura_orden = ord.id_estructura
     ...
     while ($data){
           ...
           while (($data) && ($cond == $data['legajo'])){
                 ...
                 while (($data) && ($cond == $data['legajo'])){
                    if (($data['id'])&&($id_emp == 1)){
                     /*  try{
                        $desde = new DateTime("$data[hcitacion]");
                        $hasta = new DateTime("$data[hfinservicio]");
                        if ($hasta < $desde){
