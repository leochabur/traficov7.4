...
header("Expires: 0");
  ...
  $accion= $_POST['accion'];
     ...
     $sql = "select upper(razon_social) as razon_social, upper(tu.tipo) as tipo, count(*), round(sum(peajes), 2)
             ...
             where (peajes > 0) and (fservicio between '$desde' and '$hasta') and (o.id_estructura = $_SESSION[structure]) and (not borrada) and (not suspendida) and (id_cliente_vacio is null)
             group by c.id, tu.id
             union all
