<?
  ...
  elseif($accion == 'reskm'){
     ...
     $sql = "select upper(razon_social) as cli, date_format(o.fservicio, '%d/%m/%Y') as fservicio, interno, upper(ori.ciudad), upper(des.ciudad), o.km, comentario , upper(concat(e1.apellido,', ', e1.nombre)) as emple1, upper(concat(e2.apellido,', ', e2.nombre)) as emple2
             from (select fservicio, id_cliente, id_estructura_cliente, id_micro, km, nombre, hcitacion, hfinservicio, id_servicio, id_estructura_servicio, id_ciudad_origen, id_estructura_ciudad_origen, id_ciudad_destino, id_estructura_ciudad_destino, comentario, id_chofer_1, id_chofer_2
                   from ordenes ord
                   where (ord.fservicio between '$desde' and '$hasta') and (id_estructura = $_POST[str]) and (not borrada) and (not suspendida) $cliente
             ) o
             ...
             where (cr.claseservicio_id = 4) or (cr.claseservicio_id = 7)
             order by razon_social, fservicio, o.hcitacion";
   //  die($sql);
