...
try{
     ...
     $ids.="0";
      ...
      $sql="select fservicio, c.nombre, 1 as cant, if(c.id is null, 0, c.id) as crono, dayofweek(fservicio) as dia, if(s.id_TipoServicio = 20, 0, tu.id) as tipo, o.id as orden, 
                    hour(TIMEDIFF(o.hfinservicio, o.hcitacion)) as horas, minute(TIMEDIFF( o.hfinservicio, o.hcitacion)) as minutos, id_micro as micro, tu.id_estructura
            from ordenes o
     ...
     // die($sql);
      ...
      if (!$facturacion){
        die('no encuentra '.$_POST['cli']);
           ...
           $tipos = array();
         /*  try{
           $lt  = listaTiposVehiculos($estructura, 0);    
           foreach ($lt as $ti) {
                  $tipos[$ti->getId()] = $ti;
           }*/
                                  ...
                                  if (($tarifa->existeDia($dias[$value['dia']])) && ($tarifa->existeCronograma($cronos[$value['crono']])))
                                  {
                                          ...
                                          if (!($value['micro'])){
                                              if (!isset($factura['ORDEN_SIN_INTERNO'])){
                                                $factura['ORDEN_SIN_INTERNO'] = array('art'=> 'Sin coche asignado', 'cant' => $value['cant'], 'ords' => array());
                                              }
                                              else{
                                          ...
                                          elseif (!$value['tipo']){                                              
                                              if (!isset($factura['ORDEN_SIN_TIPO_VEHICULO'])){
                                                $factura['ORDEN_SIN_TIPO_VEHICULO'] = array('art'=> 'Sin tipo asignado', 'cant' => $value['cant'], 'ords' => array());
                                              }
                                              else{
                                          ...
                                          {  ////la orden tiene tipo de vehiculo asignado
                                            die("EL tipo es ".$tipos[$value['tipo']]);
