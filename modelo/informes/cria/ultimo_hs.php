<?
  ...
  function getNovedadesFecha($fecha, $conductor, $conexcion){
           $sql="SELECT id_novedad
                 FROM novedades
                 where '$fecha' between desde and hasta and id_novedad in (16, 18) and id_empleado = $conductor";
           $res_nov = mysql_query($sql, $conexcion) or die($sql);
           if ($row = mysql_fetch_array($res_nov)){
              return $row[0];
           }
           else{
                return 0;
  ...
  function ajustarHoras($desde, $hasta, $ultima_vuelta){
           if ($hasta < $desde){
...
$conn = conexcion();
     ...
     $i=0; $j=0;
           ...
           while (($data) &&($cond == $data['cond'])){
                 ...
                 $dia_fin  = $data['dia_fin']; //utilizado para indicar el numero de dia semana y realizar o no el corte
                 $id_nove = getNovedadesFecha($data['fservicio_inicio'], $cond, $conn);   //lo primero que hace es verificar si hay aguna novedad en la fecha que se esta procesando para el condurcotr
                 if ($id_nove == 16){  //encuentra novedad de franco trabajado
                    $inicio_franco_trab = new DateTime($data['dtdesde']);   //toma el inicio de la joranda
                 ...
                 elseif($id_nove == 18){  ///encuentra novedad de feriado trabajado
                    $fecha_franco_trab = $data['fservicio_inicio'];
                    ...
                    $cant_jornadas++;
                      ...
                      //die ($dia_fin);
                      ajustarHoras($desde, $hasta, $ultima_vuelta); //si desde es mayor q hasta o utlima_vueta es menor q desde
                 
              //   if ($data['dia_inicio'] == ) //lo
                 ...
                 elseif ($desde > $corte50){
                        $minutos_falta_desc = datetomin($ultima_vuelta->diff($desde));
                        $minutos_falta_desc=720-$minutos_falta_desc;
                        if ($minutos_falta_desc > 0){
                           $minutos_al_100+= $minutos_falta_desc;
                        }
                        $resto_min=0;
