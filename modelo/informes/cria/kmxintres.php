<?
  ...
  if($accion == 'reskm'){
     ...
     $sql = "SELECT count(*), sum(km), interno, u.id, upper(razon_social) as razon_social
             FROM (select id_cliente, id_estructura_cliente, id_micro, o.id_estructura, km, razon_social
                   from ordenes o
                   inner join clientes c on c.id = o.id_cliente and c.id_estructura = o.id_estructura_cliente
                   where (fservicio between '$desde' and '$hasta') and (not suspendida) and (not borrada) and (o.id_estructura = $str) and (id_cliente_vacio is null) and (id_micro is not null)
                   union all
                   select id_cliente_vacio, id_estructura_cliente_vacio, id_micro, o.id_estructura, km, concat('VACIO - ',razon_social)
                   from ordenes o
                   inner join clientes c on c.id = o.id_cliente_vacio and c.id_estructura = o.id_estructura_cliente_vacio
                   where (fservicio between '$desde' and '$hasta') and (not suspendida) and (not borrada) and (o.id_estructura = $str) and (id_cliente_vacio is not null) and (id_micro is not null)
                          and (id_cliente_vacio <> (select valor from opciones where opcion = 'cliente-vacio' and id_estructura = $str))
                   union all
                   select id_cliente_vacio, id_estructura_cliente_vacio, id_micro, o.id_estructura, km, razon_social
                   from ordenes o
                   inner join clientes c on c.id = o.id_cliente_vacio and c.id_estructura = o.id_estructura_cliente_vacio
                   where (fservicio between '$desde' and '$hasta') and (not suspendida) and (not borrada) and (o.id_estructura = $str) and (id_cliente_vacio is not null) and (id_micro is not null)
                          and (id_cliente_vacio = (select valor from opciones where opcion = 'cliente-vacio' and id_estructura = $str))
...
group by o.id_micro, o.razon_social
order by interno";
