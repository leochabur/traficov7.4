<?
  ...
  $sql="select interno, date_format(fservicio,'%d/%m/%Y') as fecha, upper(concat(apellido,', ', nombre)) as conductor, id_chofer_1
    ...
    from ordenes
    where fservicio between '$desde' and '$hasta' and id_estructura = $_SESSION[structure] $interno and id_chofer_1 is not null
    union all
    select fservicio, id_micro, id_chofer_2
    from ordenes
    where fservicio between '$desde' and '$hasta' and id_estructura = $_SESSION[structure] $interno and id_chofer_2 is not null
    ) o
