...
include_once ('../../../modelsORM/src/OrdenFacturada.php');  
      ...
      foreach ($facturas as $factura) {
        ...
        if (!$factura->getCerrada())
        {
          ...
          $table.='<tr>
                      ...
                      <td>      
                        <a href="/vista/informes/cria/viewcert.php?fv='.$factura->getId().'" target="_blanck" title="Ver certificado"><i class="fas fa-bars fa-2x"></i></a>            
                        '.$close.'
                      </td>
                   </tr>';
      }
  ...
  elseif($accion == 'factFjo')
  {
     ...
     $ids.="0";
      $formato = 'd/m/Y';
      $desde = DateTime::createFromFormat($formato, $_POST['fecha']);
      $hasta = DateTime::createFromFormat($formato, $_POST['hasta']);    
      $desde = $desde->format('Y-m-d');
      $hasta = $hasta->format('Y-m-d');
      $sql = "SELECT idTipo, tipoServicio, km, idOrden
              FROM(
              SELECT ts.id as idTipo, ts.tipo as tipoServicio, km, o.id as idOrden
              FROM ordenes o
              inner join servicios s on s.id = o.id_servicio and s.id_estructura = o.id_estructura_servicio
              inner join tiposervicio ts on s.id_TipoServicio = ts.id AND  s.id_estructura_TipoServicio = ts.id_estructura
              where not borrada and not suspendida and o.id_cliente = $_POST[cliente]
                    and fservicio between '$desde' and '$hasta' and o.id not in ($ids)) o
              ORDER BY idTipo";
     // die($sql);
      $conn = conexcion();

      $sqlKm = "SELECT sum(km) as km
                FROM ordenes o
                inner join servicios s on s.id = o.id_servicio and s.id_estructura = o.id_estructura_servicio
                where not borrada and not suspendida and o.id_cliente = 508 and fservicio between '2020-03-01' and '2020-03-31' and o.id not in (0)";

      $resultKm = mysql_query($sqlKm, $conn) or die($sqlKm);

      $km = 0;
      if ($rowKm = mysql_fetch_array($resultKm))
      {
          $km = $rowKm['km'];
      }
      ...
      {
          if (!isset($detalle[$row['idTipo']]))
          {
            $detalle[$row['idTipo']] = array('srv' => $row['tipoServicio'], 'km' => 0, 'cant' => 0, 'ords' => array());
          }

          $detalle[$row['idTipo']]['cant']++;
          $detalle[$row['idTipo']]['km']+=$row['km'];
          $detalle[$row['idTipo']]['ords'][] = $row['idOrden'];

          
      }

      $facturacionn = facturacionCliente($_POST['cliente']);
      $tfa = $facturacionn->getTarifas()->first();
      $tarifa = $facturacionn->getTarifas()->first()->getTarifasTipoVehiculo()->first();
      $pesoskm = ($km?$tarifa->getArticulo()->getImporte()/$km:0);

      $tabla = "<table class='table table-zebra'>
                      <thead>
                        <tr>
                            <th>N#</th>
                            <th>Tipo Servicio / Nombre Servicio</th>
                            <th>Cantidad</th>
                            <th>Km</th>
                            <th>$/KM</th>
                            <th>Total</th>
      ...
      foreach ($detalle as $key => $value) {
        $total = $pesoskm * $value['km'];
        $tabla.="<tr>
                      <td>$i</td>
                      <td>$value[srv]</td>
                      <td align='right'>$value[cant]</td>
                      <td align='right'>$value[km]</td>
                      <td align='right'>$ ".number_format($pesoskm,2,',','.')."</td>
                      <td align='right'>$ ".number_format($total,2,',','.')."</td>
                      <td><input type='button' class='fatura' value='Incorporar a la factura' data-tfa='".$tfa->getId()."' data-art='".$tarifa->getArticulo()->getId()."' data-valkm='$pesoskm' data-fact='$_POST[factura]' data-ords='".implode(',', $value['ords'])."'/></td>
                  </tr>";
      }
      $tabla.="</tbody>
              </table>
              <script>
                  $('.fatura').button().click(function(){
                                                          var btn = $(this);
                                                          btn.hide();
                                                          if (confirm('Seguro agregar las ordenes a la fcatura?'))
                                                          {
                                                                $.post('/modelo/informes/cria/factvta.php',
                                                                       {accion : 'facallFijo',
                                                                        art: btn.data('art'),
  ...
  elseif ($accion == 'facallFijo') {
    try{
          ...
          foreach ($ordenes as $orden) {
              ...
              $ordeneFacturada->setCantidad(1);
              $ordeneFacturada->setArticulo($articulo);
              $ordeneFacturada->setImporteUnitario(($orden->getKm()*$_POST['prkm']));
              $ordeneFacturada->setImporte(($orden->getKm()*$_POST['prkm']));
...
//id, , horacierreinforme, ingresodesde, salidahasta, , id_estructura_cliente                                                                                LEFT JOIN tiposervicio ts ON ts.id = s.id_TipoServicio AND ts.id_estructura = s.id_estructura_TipoServicio
      $sql="select fservicio, c.nombre, 1 as cant, if(c.id is null, 0, c.id) as crono, dayofweek(fservicio) as dia, tu.id as tipo, o.id as orden, 
            ...
            where o.id_cliente = $cliente and 
                 ...
                 concat(fservicio, ' ', o.hsalida) between concat('$desde', ' ', '05:00:00') and concat('$hasta', ' ', '23:59:00') or
                 (s.i_v = 'v' and concat(fservicio, ' ', o.hsalida) between concat(DATE_ADD('$hasta', INTERVAL 1 day), ' ' ,'00:00:00') 
                 and concat(DATE_ADD('$hasta', INTERVAL 1 day), ' ' ,'07:00:00'))) and 
      ...
      $sql="select fservicio, c.nombre, 1 as cant, if(c.id is null, 0, c.id) as crono, dayofweek(fservicio) as dia, tu.id as tipo, o.id as orden, 
            ...
            from ordenes o
            left join servicios s on s.id = o.id_servicio and s.id_estructura = o.id_estructura_servicio
            left join cicloinforme ci on ci.id_tiposervicio = s.id_TipoServicio AND ci.id_estructuratiposervicio = s.id_estructura_TipoServicio AND ci.id_cliente = o.id_cliente
            left join cronogramas c on c.id = s.id_cronograma and c.id_estructura = s.id_estructura_cronograma
            left join unidades u on u.id = o.id_micro
            left join tipounidad tu on tu.id = u.id_tipounidad and tu.id_estructura = u.id_estructura_tipounidad
            where o.id_cliente = $cliente and 
                  (concat(fservicio, ' ', o.hsalida) between concat('$desde',' ', ingresodesde) and concat('$hasta',' 23:59:59')  or 
                  concat(fservicio, ' ', o.hllegada) between concat('$desde',' ', ingresodesde) and concat('$hasta',' 23:59:59')  or 
                  concat(fservicio,' ',o.hsalida) between date_add(concat('$desde',' 00:00:00'), interval 1 day) and date_add(concat('$hasta', ' ',salidahasta), interval 1 day)) 
                 and not borrada and not suspendida and o.id not in ($ids) $tiposS
            order by dayofweek(fservicio), c.id, tu.id";
      //die($sql);
                                    ...
                                    try
                                    {
                                            if (($tarifa->existeDia($dias[$value['dia']])) && ($tarifa->existeCronograma($cronos[$value['crono']])))
                                            {
                                                 ////la orden tiene tipo de vehiculo asignado
                                                  ...
                                                  if ($tfa)
                                                  { ///existe una tarifa para el tipo de vehiculo
                                                      $cant = $value['cant'];
