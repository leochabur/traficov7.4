<?php
  ...
  elseif($accion == 'reskm'){
     ...
     $sql = "select interno, date_format(fservicio, '%d/%m/%Y') as fservicio, date_format(hcitacion, '%H:%i') as hsalida, date_format(hfinservicio, '%H:%i') as hfin, 
                    upper(nombre) as reco, upper(razon_social) as cli, km, 
                     if (hos.id is null, concat(date_format(fservicio, '%d/%m/%Y'), ' ', time_format(hcitacion, '%H:%i')), date_format(hos.citacion, '%d/%m/%Y %H:%i')) as citacionSur,
                     if (hos.id is null, concat(date_format(fservicio, '%d/%m/%Y'), ' ', time_format(hfinservicio, '%H:%i')), date_format(hos.finalizacion, '%d/%m/%Y %H:%i')) as finalizaSur,
                     if (hos.id is null, date_format(hcitacion, '%d/%m/%Y'), date_format(hos.citacion, '%d/%m/%Y')) as fechaSur
             from (select fservicio, id_cliente, id_estructura_cliente, id_micro, km, nombre, hcitacion, hfinservicio, id, id_estructura
                   from ordenes ord
                   where (ord.fservicio between '$desde' and '$hasta') and (ord.id_estructura = $_POST[str]) and (not suspendida) and (not borrada) $interno
             ) o
             left join horarios_ordenes_sur hos ON hos.id_orden = o.id AND hos.id_estructura_orden = o.id_estructura
   ...
   // die($sql);
     ...
     while ($data){
           ...
           while (($data) &&($int == $data['interno'])){
               ...
               $fecha = ($_POST[str] == 2?$data['fechaSur']:$data['fservicio']);
               $salida = ($_POST[str] == 2?$data['citacionSur']:$data['hsalida']);
               $llegada = ($_POST[str] == 2?$data['finalizaSur']:$data['hfin']);
               $tabla.="<tr>
