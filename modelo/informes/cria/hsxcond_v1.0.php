<?php
  ...
  if($accion == 'hscond'){
     $minutosCorteTurno = 240; //representa los minutos que son tenidos en uant para saber si una orden viene enganchada con el turno del dia anterior o no
     $rango = "$_POST[desde]  hasta  $_POST[hasta]";
     $desde = dateToMysql($_POST['desde'], '/');
     $hasta = dateToMysql($_POST['hasta'], '/');
           $table=allDriver($desde, $hasta, $_POST['emple'], 240);
    print $table;
  }
...
function vieneDelTurnoAnterior($conn, $row, $minCorte)
{ 
  ...
  try
  {
      ...
      $now = clone $fechaActual;
      $now->sub(new DateInterval('P1D'));
      $fecha = $now->format('Y-m-d');
      $sqlLast = "SELECT concat(if (hfinservicio < hcitacion, DATE_ADD(fservicio, INTERVAL 1 DAY), fservicio),' ', hfinservicio) as dthfin
                  FROM ordenes o
                  WHERE (id_estructura = $_SESSION[structure]) and ($row[id_fercho] in (id_chofer_1, id_chofer_2)) and (fservicio = '$fecha') and (not borrada) and (not suspendida)
                  ORDER BY hcitacion DESC
                  LIMIT 1";
      $resultLast = mysql_query($sqlLast, $conn) or (die($sqlLast));
      if ($row = mysql_fetch_array($resultLast))
      {
        $lastFecha = datePHP($row['dthfin']);
  ...
  function allDriver($desde, $hasta, $empl, $min)
  {
      $condemp1="";
      $condemp2="";
      if ($empl){
      ...
      $conn = conexcion();
      $sql = getSql($desde, $hasta);
      $result = mysql_query($sql, $conn) or die("ok");
      $data = mysql_fetch_array($result);
      $arrayMins = array();
      while ($data){ //comienza a recorrer
        try{
                 $cond = $data['cond'];
                 $ultOrden = '';
                 while (($data) && ($cond == $data['cond'])){ // mientras se procese al mismo conductor debe continuar
                    $dtInicio = datePHP($data['dtdesde']);
                    $dtFin = datePHP($data['dthasta']);
                    if ($ultOrden) ///ya ha procesado al menos una orden correspondiente al conductor actual
                    {

                    }
                    else{ //Es la primer orden que se procesa, debe evaluar si corresponde o no al turno del dia anterior

                        $continuaTurnoAnterior = vieneDelTurnoAnterior($conn, $data, $min); ///al ser la primera orden que procesa debe identificar si la misma es continuacion del turno del dia anterior
                        if (!$continuaTurnoAnterior)//no viene del turno anterior
                        { 
                          $minutos = datetomin2($dtInicio->diff($dtFin));
                          $arrayMins = seccionarMinutos($minutos);
                        }
                    }
                    $ultOrden = $data;
                    $data = mysql_fetch_array($result);
            ...
            catch (Exception $e){return ("es mi amigo el bandolete ".$e->getMessage());}
      }
      return $tabla;
  }


  function getSql($desde, $hasta){
      $sql = "select o.*, concat(apellido,', ', nombre) as emple, legajo, 
                    concat(fservicio_inicio,' ',hcitacion) as dtdesde, concat(fservicio_fin,' ',hfinservicio) as dthasta, 
  ...
  function seccionarMinutos($minutos){
    $horas =array('hsNormales' => $minutos, 'hs50' => 0, 'hs100' => 0);
    if ($minutos > 480){ 
        $horas['hsNormales'] = 480;
        $minutos-=480;
        if ($minutos >= 240){
          $horas['hs50'] = 240;
          $minutos-=240;
