...
elseif($accion=='addrstgral'){
     ...
     $response = array();     ///respuesta al cliente de la accion requerida
     $response[status] = true;
     $campos ="ant_max, susp_neum, susp_elas, motor_tras, motor_del, id_cliente, id_estructuraCliente, filtralistaunidades, admitefletero";
     $values="$_POST[antmax], $_POST[susne], $_POST[susela], $_POST[mottras], $_POST[motdel], $_POST[cli], $_POST[str], $_POST[flista], $_POST[afl]";
     try{
         $sql = "INSERT INTO restricciongralcliente ($campos) VALUES ($values)
                 ON DUPLICATE KEY UPDATE ant_max=$_POST[antmax], susp_neum=$_POST[susne], susp_elas=$_POST[susela], motor_tras=$_POST[mottras], motor_del=$_POST[motdel], filtralistaunidades=$_POST[flista], admitefletero = $_POST[afl]";
         mysql_query($sql, $conn);
         $response[msge] = "Restriccion modificada con exito!!";
         cerrarconexcion($conn);
         print json_encode($response);
