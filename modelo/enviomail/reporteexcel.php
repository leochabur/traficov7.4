<?php
  ...
  $consulta = "select ts.tipo,
       tu.tipo,
       if(i_v = 'i', ori.ciudad, d.ciudad) as localidad,
       cast(o.nombre as char),
           interno,
       time_format(if(i_v = 'i', hllegadaplantareal, hsalidaplantareal), '%H:%i') as horario,
       cantpax,
...
left join fact_cronogramas_por_tarifa cxt on cxt.id_cronograma = c.id and c.id_estructura = o.id_estructura
left join (select * from fact_tarifa_tipo_servicio where defecto) tts on tts.id_tarifa_servicio = cxt.id_tarifa_servicio
left join fact_articulos_clientes ac on ac.id = tts.id_articulo
...
left join fact_lineas_planillas lp ON ((s.id = id_servicio_entrada AND s.id_estructura = id_estructura_servicio_entrada) OR (s.id = id_servicio_salida AND s.id_estructura = id_estructura_servicio_salida))
where 
    ...
    concat(fservicio,' ',o.hsalida) between date_add(concat('$fecha',' 00:00:00'), interval 1 day) and date_add(concat('$fecha', ' ',salidahasta), interval 1 day))
      and o.id_estructura = $_SESSION[structure] and o.id_cliente = 10 and not borrada and not suspendida
order by ts.tipo, i_v, horario";
  //  die($consulta);
	...
	if(mysql_num_rows($result)){
		...
		$titulosColumnas = array('TIPO SERVICIO', 'FECHA SERVICIO', 'LOCALIDAD', 'RECORRIDO (MASTER BUS)', 'RECORRIDO (TOYOTA)', 'ARTICULO', 'INTERNO', 'HORARIO', 'PASAJEROS');
		...
		while ($fila = mysql_fetch_array($result)) {
			$objPHPExcel->setActiveSheetIndex(0)
                ...
                ->setCellValue('E'.$i, $fila['nombreLinea'])
...
//		exit;
	...
	//	$objWriter->save(str_replace(__FILE__,'/reporteservicios.xlsx',__FILE__));
         ...
         $dest = 'talcantara@masterbus.net, dscirocco@masterbus.net, leochabur@gmail.com';
