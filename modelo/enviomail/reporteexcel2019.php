...
left join unidades u on u.id = o.id_micro
left join tipounidad tu on tu.id = u.id_tipounidad and tu.id_estructura = u.id_estructura_tipoUnidad
where concat(fservicio, ' ', o.hsalida) between date_sub(concat('$fecha',' ',horacierreinforme), interval 1 day) and concat('$fecha', ' ',horacierreinforme)
...
order by ts.tipo, i_v, horario";
  //  die($consulta);
	...
	if(mysql_num_rows($result)){
		...
		$titulosColumnas = array('TIPO SERVICIO', 'FECHA SERVICIO', 'LOCALIDAD', 'RECORRIDO', 'ARTICULO', 'INTERNO', 'HORARIO', 'PASAJEROS');
