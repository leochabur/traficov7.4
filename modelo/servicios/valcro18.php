...
elseif($accion =='addart'){
    $sql = "INSERT INTO articulosClientes (articulo, id_cliente, id_estructura_cliente) VALUES ('$_POST[desc]', $_POST[clientes], $_SESSION[structure])";
    $response = array('ok' => true, 'message'=>'');

    if (!$_POST[desc]){
       $response['ok'] = false;
       $response['message'] = 'El articulo no se puede guardar sin una descripcion!!!';
    ...
    if (mysql_errno($conn)){
       $response['ok'] = false;
       $response['message'] = 'Error al intentar guardar el articulo!!';
    }
    mysql_close($conn);
