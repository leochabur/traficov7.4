 ...
 <script>
	$(function() {
		...
		$("#pxd").button().click(function(){
                                                $.post("/modelo/segvial/vtocauda.php", {accion: 'pxday', uda: $('#unidades').val(), dias: $('#dias').val()}, function(data) {$('#table').html(data);});
                                                });
		$("#rfe").button().click(function(){
                                                $.post("/modelo/segvial/vtocauda.php", {accion: 'rafec', uda: $('#unidades').val(), hasta: $('#hasta').val(), desde: $('#desde').val()}, function(data) {$('#table').html(data);});
                                                });
	});
	</script>
	...
	<div id="tabs-1" class="ui-state-highlight ui-corner-all">
         <form class="cmxform" id="commentForm" method="get" action="">
	           <fieldset class="ui-widget ui-widget-content ui-corner-all">
		                 <legend class="ui-widget ui-widget-header ui-corner-all">Vencimientos Polizas de Seguro Internos</legend>
		                 <div id="mensaje"></div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <select id="unidades" name="unidades" class="ui-widget-content  ui-corner-all"  validate="required:true">
                                                ...
                                                <?php
                                                  armarSelect('unidades', 'CAST(interno as UNSIGNED)', 'id', 'interno', "activo");
                                                ?>
                                        </select>
                                    </td>
                                    <td>
