                     ...
                     <tbody>
                            <?php
                                 ...
                                 if ($cantTripulacion > 2){
                                    ...
                                    $join_horarios = "LEFT JOIN horarios_ordenes_sur hhs ON hhs.id_orden = o.id and hhs.id_estructura_orden = o.id_estructura";
                                    $where = "(('$fecha' between date(citacion) and date(finalizacion)) or (fservicio = '$fecha'))"; 
                                    $cita = "if (citacion is null, o.hcitacion,if (date(citacion) < '$fecha', '00:00:00', citacion))";
                                    $sale = "if (salida is null, o.hsalida, if (date(salida) < '$fecha', '00:00:00', salida))";
                                    $fina = "if (finalizacion is null, o.hfinservicio, if (date(finalizacion) > '$fecha', '23:59:59', finalizacion))";
                                    $llega = "if (llegada is null, o.hllegada, if (date(llegada) > '$fecha', '23:59:59', llegada))";
                                 }

                                 $sql = "SELECT o.id, finalizada, time_format($cita, '%H:%i') as hcitacion, time_format($sale, '%H:%i') as hsalida, 
                                                time_format($fina, '%H:%i') as hfinserv, o.nombre, if(em1.id = 1,concat(ch1.apellido, ', ',ch1.nombre), concat('(',em1.razon_social,') ', ch1.apellido, ', ',ch1.nombre)) as chofer1, upper(c.razon_social) as razon_social, concat(ch2.apellido, ', ',ch2.nombre) as chofer2, os.comentario as comentario, interno, ch1.id_empleado, ch2.id_empleado as id_empleado2, suspendida, checkeada, emp.color, date_format($llega, '%H:%i') as hllegada, ot.id as or_tur, cantpax, i_v, 1 as is_orden, date_format(o.hllegadaplantareal, '%H:%i') as hllegadaReal, date_format(o.hsalidaplantareal, '%H:%i') as hsalidaReal, op.id as cliVac $camposTripulacion
                                                       FROM ordenes o
                                                       $join_horarios
