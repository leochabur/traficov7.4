<?
  ...
  $sql = "SELECT o.id_cliente FROM ordenes o WHERE o.id = $orden";
 ...
 // if ($_SESSION['structure'] == 1){
  $sql = "select id_coche, interno, 1 as disponible
          ...
          inner join (select * from unidades where id_estructura = $est) u on u.id = r.id_coche
          ...
          from unidades u
          where activo and id_estructura = $est
 ...
 // }
   ...
   while ($data){
         ...
         while (($data) && ($disp == $data[disponible])){
               if ($disp){
                  $option.="<option style='background-color: $back' class='$ok' value='$data[id_coche]'>$data[interno]</option>";
