 ...
 <script>
	...
	function duplicarOrden(id){
          if(confirm('Seguro duplicar la orden?'))
                             $.post('/modelo/procesa/mod_ordenes.php', {accion: "dupli", orden: id}, function(data){
                     ...
                     <tbody>
                            <?php
                                 ...
                                 $sql = "SELECT o.id, finalizada, date_format(hcitacion, '%H:%i') as hcitacion, date_format(hsalida, '%H:%i') as hsalida, date_format(hfinservicio, '%H:%i') as hfinserv, o.nombre, if(em1.id = 1,concat(ch1.apellido, ', ',ch1.nombre), concat('(',em1.razon_social,') ', ch1.apellido, ', ',ch1.nombre)) as chofer1, upper(c.razon_social) as razon_social, concat(ch2.apellido, ', ',ch2.nombre) as chofer2, comentario, interno, ch1.id_empleado, ch2.id_empleado as id_empleado2, suspendida, checkeada
                                                       ...
                                                       WHERE (fservicio = '$fecha') and (not borrada) and (o.id_estructura = $_SESSION[structure]) and (finalizada in ($_SESSION[todas]))
                                         UNION
                                         SELECT n.id as id, 0 as finalizada, '00:00' as hcitacion, '00:00' as hsalida, '00:00' as hfinserv, cn.nov_text as nombre, concat(e.apellido, ', ',e.nombre) as chofer1, '' as razon_social, '' as chofer2, '' as comentario, '' as interno, e.id_empleado as id_empleado, 0 as id_empleado2, 0, 0
                                         FROM novedades n
                                         inner join cod_novedades cn on cn.id = n.id_novedad
                                         inner join empleados e on e.id_empleado = n.id_empleado
                                         where ('$fecha' between desde and hasta) and (n.id_estructura = $_SESSION[structure]) and (e.id_cargo = 1)
                                         order by hcitacion";
				                 ...
				                 while($row = mysql_fetch_array($query)){
					                        ...
					                        print "<tr>
                                                       ...
                                                       <td class=\"$tdclass\"><div class=\"$divclass\" id=\"nombre-$id\">".htmlentities($row['nombre'])."</div></td>
                                                       <td class=\"$tdclass\"><div class=\"$divclass\" id=\"id_micro-$id\"><a href='/vista/segvial/posint.php?int=$row[interno]' target='_blank' onClick=\"window.open(this.href, this.target, 'width=900,height=650'); return false;\">$row[interno]</a></div></td>
                                                       ...
                                                       <td class=\"$tdclass\"><div class=\"$divclass\" id=\"id_diagnostico-$id\">".htmlentities($row['razon_social'])."</div></td>
