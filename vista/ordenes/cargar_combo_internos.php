<?php
  ...
  $sql = "SELECT o.id_cliente, o.id_estructura_cliente, filtralistaunidades as flista, admitefletero as adfl
          FROM ordenes o
  ...
  if ($data = mysql_fetch_array($result)){
     ...
     if ($data[adfl]){
        $fletero = "";
          ...
          $fletero = " and (id_propietario = 1)";
     }

//(if (ant_max is null, true ,(year(now()) - anio) <= ant_max)) and 
//(if (ant_max is null, true ,(year(now()) - anio) <= ant_max)) and
     if ($data[flista]){
          $flista="SELECT u.id as id_coche, interno, id_tipounidad, id_estructura_tipounidad, r.id_cliente, r.id_estructuracliente, anio
                   FROM restcochesclientes r
                   inner join (select * from unidades where id_estructura = $str $fletero) u on u.id = r.id_coche
                   left join restricciongralcliente rgc on rgc.id_cliente = r.id_cliente and rgc.id_estructuraCliente = r.id_estructuracliente
                   where permitido and r.id_cliente = $cliente and activo";
     }
     else{
          $flista="SELECT u.id as id_coche, interno, id_tipounidad, id_estructura_tipounidad, $cliente as id_cliente, $str as id_estructuracliente, anio
                   FROM (select * from unidades where id_estructura = $str $fletero) u
     ...
     $sql = "select id_coche, interno, 1 as disponible
          ...
          inner join (select * 
                     ...
                     where id_tipovto is null) rctu on rctu.id_cliente = o.id_cliente and o.id_estructuracliente = rctu.id_estructuracliente 
                                                       and rctu.id_tipounidad = o.id_tipounidad and rctu.id_estructura_tipounidad = o.id_estructura_tipounidad
          where (if (antiguedad is null, true ,(year(now()) - anio) <= antiguedad))
          union all
      ...
      {
        $sql="SELECT u.id as id_coche, if (emp.id = 1, interno, concat(interno,' (',razon_social,')')) as interno, 0 as disponible
FROM unidades u
inner join empleadores emp on emp.id = u.id_propietario where u.id_estructura = $_SESSION[structure] and u.activo ORDER BY interno";
