...
<script type="text/javascript" src="<?php echo RAIZ;?>/vista/js/validate-form/jquery.metadata.js"></script>
 <script>
	...
	$(function() {
                   ...
                   $(".ocultar").hide();
                  $(".tsmo").click(function(){
                                              var id_orden = $(this).attr("id").split("-");
                                              ...
                                              dialog.load('/vista/ordenes/ordturview.php',{orden:id_orden[1], cond:''},function (responseText, textStatus, XMLHttpRequest) {dialog.removeClass('loading');});
                                              });
                     ...
                     <tbody>
                            <?php
                                 ...
                                 /*                                         union
                                         ...
                                         where vigente and id_estructua_destino = $_SESSION[structure] and fservicio = '$fecha'*/
                                 ...
                                 $sql = "SELECT o.id, finalizada, date_format(o.hcitacion, '%H:%i') as hcitacion, date_format(o.hsalida, '%H:%i') as hsalida, date_format(o.hfinservicio, '%H:%i') as hfinserv, o.nombre, if(em1.id = 1,concat(ch1.apellido, ', ',ch1.nombre), concat('(',em1.razon_social,') ', ch1.apellido, ', ',ch1.nombre)) as chofer1, upper(c.razon_social) as razon_social, concat(ch2.apellido, ', ',ch2.nombre) as chofer2, os.comentario as comentario, interno, ch1.id_empleado, ch2.id_empleado as id_empleado2, suspendida, checkeada, emp.color, date_format(o.hllegada, '%H:%i') as hllegada, ot.id as or_tur, cantpax, i_v, 1 as is_orden, date_format(o.hllegadaplantareal, '%H:%i') as hllegadaReal, date_format(o.hsalidaplantareal, '%H:%i') as hsalidaReal, op.id as cliVac $camposTripulacion
                                                       FROM ordenes o
                             ...
                             //    die($sql);
				                 ...
				                 while($row = mysql_fetch_array($query)){
                                            $obs=""; $hora=""; $cond=""; $tdclass="fin"; $divclass=""; //inicializamos todas las clases correspondientes a los estilos a aplicar
                                            $tdclass="";
                                            if ($row['finalizada']){
                                            ...
                                            $hstd = "<";
                                            $horarioSTD="<td $tdclass></td>
                                                            <td $tdclass></td>
                                                            <td $tdclass></td>";
                                            if ($row[i_v] == 'i'){
                                               $hstd = $row[hllegada];
                                               $horarioSTD="<td $tdclass align='center'></td>
                                                            <td $tdclass><div align='center'>$hstd</div></td>
                                                            <td $tdclass align='center'><input class='hora hreal' type='text' size='5' value='$row[hllegadaReal]' id='hllegadaplantareal-$id' alt='Hora de llegada Real'></td>";
                                            }
                                            elseif ($row[i_v] == 'v'){
                                                   $hstd = $row[hsalida];
                                                   $horarioSTD="<td $tdclass align='center'><input class='hora hreal' type='text' size='5' value='$row[hsalidaReal]' id='hsalidaplantareal-$id' alt='Hora de salida Real'></td>
                                                                <td $tdclass><div align='center'>$hstd</div></td>
                                                                <td $tdclass align='center'></td>";
                                            }
                                            ...
                                            $img='';
                                            if ($row['chofer2'])
                                               $img = "<img title='Ver diagrama de ".($row['chofer2'])."' src='../user.png' border='0' onclick='mostrarDiag($row[id_empleado2]);' style='cursor:pointer'>";
                                            $dri='';
                                            if ($row['chofer1'])
                                               $dri="<img title='Ver diagrama de ".($row['chofer1'])."' src='../user.png' border='0' onclick='mostrarDiag($row[id_empleado]);' style='cursor:pointer'>";
                                            if ($cantTripulacion > 2){
                                               $dri3="";
                                               if ($row['chofer3']){
                                               ...
                                               $dri3="<td class=\"$tdclass\"><div align='center'>$dri3</div></td>";
                                            }
                                            $turi="";
                                            if ($row[or_tur]){
                                               $turi = "tsmo";
                                           ...
                                           // if (($_SESSION['userid'] == 25) || ($_SESSION['userid'] == 18)|| ($_SESSION['userid'] == 37))
					                        ...
					                        $body.="<tr class='headt'>
                                                       <td $tdclass><div class='$hora' id='hcitacion-$id'>$row[hcitacion]</div></td>
                                                       <td $tdclass><div class='$hora' id='hsalida-$id'>$row[hsalida]</div></td>
                                                       <td $tdclass><div class='$divclass $turi' id='nombre-$id'>".$row['nombre']."</div></td>
                                                       <td $tdclass><div class='$divclass' id='id_micro-$id'><a href='/vista/segvial/posint.php?int=$row[interno]' target='_blank' onClick=\"window.open(this.href, this.target, 'width=900,height=650'); return false;\"><div style='color:#$row[color];'>$row[interno]</div></a></div></td>
                                                       <td $tdclass><div class='$divclass' id='id_chofer_1-$id' style='cursor: pointer;' onclick='mostrarDiag($row[id_empleado]);'>".($row['chofer1'])."</div></td>
                                                       <td $tdclass><div class='$divclass' id='id_chofer_2-$id' style='cursor: pointer;' onclick='mostrarDiag($row[id_empleado2]);'>".($row['chofer2'])."</div></td>";
                                            if ($cantTripulacion > 2){
                                               $body.="<td $tdclass><div class='$divclass' id='id_chofer_3-$id'>".($row['chofer3'])."</div></td>";
                                            }
                                            $body.="<td $tdclass><div class='$divclass' id='razon_social-$id'>".($row['razon_social'])."</div></td>
                                                       <td $tdclass><div class='$obs $divclass' id='comentario-$id-updcmt'>$row[comentario]</div></td>
                                                       $horarioSTD
                                                       <td $tdclass><input class='hreal $cargapax' type='text' size='2' value='$row[cantpax]' id='cantpax-$id' alt='Cant. Pax.'></td>
                                                       <td $tdclass><div class='$hora' id='hfinservicio-$id'>$row[hfinserv]</div></td>
                                                       $dri3
                                                   </tr>";
                                 }
                                 print $body;
