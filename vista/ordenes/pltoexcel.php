...
elseif ($accion == 'rxls')
{
    ...
    $excel->setActiveSheetIndex(0)->setCellValue('A1', 'Etiquetas de Fila');
    $excel->setActiveSheetIndex(0)->setCellValue('B1', 'Suma de CtaMicro');
    $excel->setActiveSheetIndex(0)->setCellValue('C1', 'Costo Unitario');
    $excel->setActiveSheetIndex(0)->setCellValue('D1', 'Suma de Costo');
	...
	foreach ($resumen as $key => $value) {
		...
		{
		    $excel->setActiveSheetIndex(0)->setCellValue("A$i", "$value[nombre]");
		    $excel->setActiveSheetIndex(0)->setCellValue("B$i", '');
		    $excel->setActiveSheetIndex(0)->setCellValue("C$i", '');
			...
			foreach ($value['detalle'] as $k => $v) {
			    ...
			    $excel->setActiveSheetIndex(0)->setCellValue("A$i", "$k");
			    $excel->setActiveSheetIndex(0)->setCellValue("B$i", "$v");
			    $excel->setActiveSheetIndex(0)->setCellValue("C$i", number_format($art->getImporte(),2,',','')."");
			    $excel->setActiveSheetIndex(0)->setCellValue("D$i", number_format(($v*$art->getImporte()),2,',','')."");
				$i++;
			}
			    $excel->setActiveSheetIndex(0)->setCellValue("A$i", "TOTALES");
			    $excel->setActiveSheetIndex(0)->setCellValue("B$i", "$cant");
			    $excel->setActiveSheetIndex(0)->setCellValue("C$i", "");
			    $excel->setActiveSheetIndex(0)->setCellValue("D$i",number_format($pesos,2,',','.')."");
			    $i++;
		}
	}
	if (count($servicios))
	{
		...
		$excel->setActiveSheetIndex(0)
	            ->mergeCells("A$i:D$i");
	    $excel->setActiveSheetIndex(0)->setCellValue("A$i", "Servicios No Asignados");
	    $i++;
		foreach ($servicios as $value) 
		{
		    $excel->setActiveSheetIndex(0)->setCellValue("A$i", "$value[nombre]");
		    $excel->setActiveSheetIndex(0)->setCellValue("B$i", "$value[cant]");
		    $excel->setActiveSheetIndex(0)->setCellValue("C$i", "");
		    $excel->setActiveSheetIndex(0)->setCellValue("D$i","");
		    $i++;
		}
	}
	$excel->getActiveSheet()->getStyle('A1:D'.$i)->applyFromArray($styleArray);
	header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
	header('Content-Disposition: attachment;filename="Resumen_servicios_'.$fecha.'.xlsx"');
	header('Cache-Control: max-age=0');
