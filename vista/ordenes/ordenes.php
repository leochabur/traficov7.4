...
<script type="text/javascript" src="<?php echo RAIZ;?>/vista/js/validate-form/jquery.metadata.js"></script>
 <script>
	...
	$(function() {
                 ...
                 $("#dialog-ch-interno, #dialog-form").dialog({autoOpen: false, width: 450, modal: true});
                     ...
                     <tbody>
                            <?php
                                 $sql = "SELECT o.id, finalizada, time_format(o.hcitacion, '%H:%i') as hcitacion, time_format(o.hsalida, '%H:%i') as hsalida, 
                                                ...
                                                date_format(o.hcitacionreal, '%H:%i') as hcitacionreal, cron.tipoServicio as isCharter
                                         FROM ordenes o
                                         LEFT JOIN servicios s on s.id = o.id_servicio and s.id_estructura = o.id_estructura_servicio
                                         LEFT JOIN cronogramas cron ON cron.id = s.id_cronograma and cron.id_estructura = s.id_estructura_cronograma
				                 ...
				                 while($row = mysql_fetch_array($query)){
                                            ...
                                            $nombreCampoHSalidaStd = ""; 
                                            $nombreCampoHLlegadaStd = "";    
                                            $nombreCitacion = "";                                                        
                                            if ($diagramaFinalizado){
                                                ...
                                                $tdSalida = "<div $tdclass class='".($row[is_orden]?'horareal':'')."' id='hsalidaplantareal-$id'>$sale</div>";
                                                $nombreCampoHSalidaStd = "hsalidaplantareal"; 
                                                $nombreCampoHLlegadaStd = "hllegadaplantareal";       
                                                $nombreCitacion = "hcitacionreal";     
                                                $nombreFinServ = "hfinservicioreal";                                   
                                            }
                                            else
                                            {
                                                $llega = $row['hllegada'];
                                                ...
                                                $sale = $row['hsalida'];
                                                $fina = $row['hfinserv'];  
                                                $tdSalida = "<div $tdclass class='".($row[is_orden]?'horareal':'')."' id='hsalida-$id'>$sale</div>";      
                                                $nombreCampoHSalidaStd = "hsalida"; 
                                                $nombreCampoHLlegadaStd = "hllegada";  
                                                $nombreCitacion = "hcitacion";
                                                $nombreFinServ = "hfinservicio";  
                                            }
                                            ...
                                            if ($row[i_v] == 'i'){
                                               ...
                                               $horarioSTD="<td $tdclass align='center' id='tdBaseWhite-$id'></td>
                                                            ...
                                                            <td $tdclass id='tdLlegadaPlantaReal-$id' align='center'><input type='text' name='hreal' size='5' value='$llega' id='$nombreCampoHLlegadaStd-$id' alt='Hora de llegada Real'></td>";
                                            }
                                            elseif ($row[i_v] == 'v'){
                                                   $hstd = $row['hsalida'];
                                                   $horarioSTD="<td $tdclass align='center' id='tdBaseWhite-$id'><input type='text' name='hreal' size='5' value='$sale' id='$nombreCampoHSalidaStd-$id' alt='Hora de salida Real'></td>
                                                                <td $tdclass id='tdBaseBlack-$id'><div align='center' id='tdBaseSubBlack-$id'>$hstd</div></td>
                                                                <td $tdclass id='tdLlegadaPlantaReal-$id' align='center'></td>";
                                            }
					                        $body.="<tr data-id='$id' style='height: 20px;' name='$name'>
                                                       <td $tdclass id='tdBaseHCitacion-$id'><div class='".($row[is_orden]?'horareal':'')."' id='$nombreCitacion-$id'>$cita</div></td>
                                            ...
                                            $body.="<td $tdclass id='tdBaseCliente-$id'><div id='razon_social-$id'>".($row['razon_social'])."</div></td>
                                                       ...
                                                       $horarioSTD
                                                       $isOrden
                                                       <td $tdclass id='tdBaseHFinServReal-$id'><div id='$nombreFinServ-$id' class='".($row[is_orden]?'horareal':'')."'>$fina</div></td>
                                                   </tr>";
                                 }
                                 print $body;
    ...
    v34(function(){
        v34.contextMenu({
            ...
            callback: function(action, options) {
                ...
                {
                    element = elem;
                    if (confirm('Esta seguro de eliminar la orden '+element))
                    {
                      $.post("/modelo/procesa/procesar_ordenes.php", 
                            {accion: 'deor', order: element}, 
                            function(data) 
                            {
                                          var response = $.parseJSON(data);
                                          if (response.ok)
