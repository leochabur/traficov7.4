...
elseif($accion == 'ldlnbl'){
	try{
		...
		foreach ($servicios as $value) {
			...
			$options.="<option value='".$value->getId()."'>".strtoupper($value->getCronograma())." - ".$hora->format('H:i')."</option>";
		}
		...
		foreach ($bloque->getLineas() as $line) {
				...
				$out="";
				if ($line->getEntrada())
					$in = $line->getEntrada()->getCronograma()." - ".$line->getEntrada()->getLlegada()->format('H:i');
				if ($line->getSalida())
					$out = $line->getSalida()->getCronograma()." - ".$line->getSalida()->getSalida()->format('H:i');				
				$tabla.="<tr>
