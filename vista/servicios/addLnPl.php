...
else
{
  ...
  $idLinea    = $data[1]; // id del servicio

  $sentido = $data[0]; //Entrada o Salida

  $idServicio = $_POST['value']; // valor por el cual reemplazar
  ...
  if ($servicio)
  {
  	$linea = $entityManager->find('LineaPlanilla', $idLinea);
  	if ($sentido == 'in')
  	{
  		$linea->setEntrada($servicio);
  	}
  	else
  	{
  		$linea->setSalida($servicio);
  	}
  	$entityManager->flush();

	if ($servicio->getIdaVelta() == 'i'){
		$hora = $servicio->getLlegada();
	}
	else{
		$hora = $servicio->getSalida();
	}

	print strtoupper($servicio->getCronograma())." - ".$hora->format('H:i')." (".$servicio->getId().")";
  }
  else
  {
  	print "error ".$idServicio;
  }
  exit;
}
	...
	try{
		...
		foreach ($bloque->getLineas() as $line) {
				...
				$tabla.="<tr>
							...
							<td>
								<div id='in-".$line->getId()."' class='changeSrv'>
									$in
								</div>
							</td>
							...
							<td>
								<div id='out-".$line->getId()."' class='changeSrv'>
									$out
								</div>
							</td>
						</tr>";
		...
		$tabla.="<form id='formLine' action='/vista/servicios/addLnPl.php'>
				 ...
				 <script>
                    $('.changeSrv').editable('/vista/servicios/addLnPl.php', {
                                                                               data   : ".json_encode($serviciosList).",
                                                                               type   : 'select',
                                                                               submit : 'Cambiar'
                                                                                });
