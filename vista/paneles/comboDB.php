<?php
     ...
     if ($accion == 'lcr'){
        ...
        $select.= '</select>
                  <script>
                          $("#servicios").change(function(){
                                                            ...
                                                            $.post("../paneles/comboDB.php",{crono: cronos, accion: \'lhr\', ctr:'.$_POST[ctr].'},function(data){
        ...
        $select.= '</select>
                   <script>
                          $("#horarios").change(function(){
                                                            ...
                                                            var fecha = $("#fservicio").val();
                                                            $.post("../paneles/comboDB.php",{hour: svr, fec:fecha, accion: \'cghr\', ctr:'.$_POST[ctr].'},function(data){
                                                                                                                                         if (data == 0){
                                                                                                                                            $(".hora").val(\'\');
                                                                                                                                         }
                                                                                                                                         else{
        ...
        $conn = conexcion();
        if ($_POST[ctr] > 2){
           $sql = "SELECT concat(concat(date_format('$fecha', '%d/%m/%Y'),' ', time_format(hcitacion, '%H:%i')),',',
concat(date_format('$fecha', '%d/%m/%Y'),' ', time_format(hsalida, '%H:%i')),',', date_format(addtime(concat('$fecha ', hsalida), tiempo_viaje), '%d/%m/%Y %H:%i'),',',
date_format(addtime(concat('$fecha ', hsalida), tiempo_viaje), '%d/%m/%Y %H:%i'),',', km) as horarios
FROM servicios s
...
WHERE (s.id = $_POST[hour]) and (s.id_estructura = $_SESSION[structure])";
        ...
        $sql="SELECT concat(date_format(hcitacion, '%H:%i,'),
                            date_format(hsalida, '%H:%i,'),
                            date_format(hllegada, '%H:%i,'),
                            date_format(hfinserv, '%H:%i,'),c.km,'') as horarios
              FROM servicios s
              INNER JOIN cronogramas c on ((c.id = s.id_cronograma)and(c.id_estructura = s.id_estructura_cronograma))
              WHERE (s.id = $_POST[hour]) and (s.id_estructura = $_SESSION[structure])";   }
        $result = mysql_query($sql, $conn);
