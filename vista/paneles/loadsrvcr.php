<?php
     ...
     if ($accion == 'lcr'){
        ...
        $sql="SELECT c.id, upper(c.nombre) as nombre, km, date_format(tiempo_viaje, '%H:%i') as tiempo, upper(o.ciudad) as origen, upper(d.ciudad) as destino, upper(cs.clase) as clase
              ...
              WHERE (c.id_estructura = $_SESSION[structure]) and (c.activo) and ((c.id_cliente = $cliente) and (c.id_estructura_cliente = $_SESSION[structure]))
              ORDER BY c.nombre";
        ...
        $crono = $_POST['crono'];
        $clases = array();
        $result = mysqli_query($conn, "SELECT id, clase FROM claseservicio WHERE id_estructura = $_SESSION[structure]");
        while ($data = mysqli_fetch_array($result)){
              $clases[$data[0]]=$data[1];
        }
        $result = mysqli_query($conn, "SELECT id, ciudad FROM ciudades where id_estructura = $_SESSION[structure] order by ciudad");
        while ($data = mysqli_fetch_array($result)){
        ...
        $estructuras = array();
        $estructuras[0]="";
        $result = mysqli_query($conn, "SELECT id, nombre FROM estructuras e where id <> $_SESSION[structure] order by nombre");
        while ($data = mysqli_fetch_array($result)){
              $estructuras[$data[0]]=$data[1];
        }
        ...
        if ($data = mysqli_fetch_array($result))
        {
          ...
          {
            $tipoCode = 'charter';
            $tipoNombre = 'Servicio con Reserva';
          }
        ...
        $sql="SELECT s.id, date_format(hcitacion, '%H:%i') as citacion, date_format(hsalida, '%H:%i') as salida, date_format(hllegada, '%H:%i') as llegada,
                     date_format(hfinserv, '%H:%i') as hfin, upper(tipo) as tipo, upper(turno) as turno, if (i_v = 'i', 'IDA', 'VUELTA') as i_v, s.activo as activo,
                     es.nombre as est_destino
              FROM servicios s
              ...
              left join controlexternoservicios ce ON ce.id_servicio = s.id and  ce.id_estructura_servicio = s.id_estructura
              left join estructuras es on es.id = ce.id_estructua_destino
              WHERE (s.id_estructura = $_SESSION[structure]) and ((s.id_cronograma = $crono) and (s.activo) and (s.id_estructura_cronograma = $_SESSION[structure]))
              order by hcitacion";
        ...
        while ($data = mysqli_fetch_array($result)){
              ...
              $select.="<tr id='$data[id]'>
                            ...
                            <td><div id='est_dest-$data[id]' class='updce'>".$data['est_destino']."</div></td>
              ...
              <form id="upcontact">
                   ...
                   <script>
                           $(function(){
                                        ...
                                        $(".updce").editable("/modelo/procesa/servicios/upd-clase.php", {
                                                                                                   data   : '.json_encode($estructuras).',
                                                                                                   type   : "select",
                                                                                                   submit : "Cambiar"
                                                                                                    });
