...
<script type="text/javascript">
                          $(document).ready(function(){
                                                       ...
                                                       $("#cargar").button().click(function(){
                                                                                              ...
                                                                                              $.post("/modelo/informes/cria/efcintcnt.php", {accion:'reskm', desde: $('#desde').val(), hasta: $('#hasta').val(), emp:$('#emples').val()}, function(data){  $('#dats').html(data);
                                                                                                                                                                           });
                                                       });
    ...
    <br><br>
         <form id="upuda">
	           <fieldset class="ui-widget ui-widget-content ui-corner-all">
		                 <legend class="ui-widget ui-widget-header ui-corner-all">Eficiencia Internos Contratados</legend>
		                 <div id="mensaje"> </div>
		                 <fieldset class="ui-widget ui-widget-content ui-corner-all">
                                        ...
                                        <select id="emples" name="emples" class="ui-widget ui-widget-content  ui-corner-all">
                                                <?php
                                                     $sql = "SELECT id, upper(razon_social)
                                                             FROM empleadores e
                                                             where activo and id_estructura = $_SESSION[structure] and id not in (SELECT valor FROM opciones where opcion = 'empleador-master' and id_estructura = $_SESSION[structure])
                                                             order by razon_social";
                                                     $result = ejecutarSQL($sql);
                                                     while ($data = mysql_fetch_row($result)){
                                                           print "<option value=\"$data[0]\">".htmlentities($data[1])."</option>";
                                                     }
                                                ?>
                                        </select>
                                    </td>
