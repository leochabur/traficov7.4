...
<script type="text/javascript">
                          $(document).ready(function(){
                                                       ...
                                                       $("#cargar").button().click(function(){
                                                                                              ...
                                                                                              $.post("/modelo/informes/cria/factmencli.php", {type:$('#type').val(),accion:'reskm', desde: $('#desde').val(), hasta: $('#hasta').val(), str: $('#str').val(), cli:$('#clientes').val()}, function(data){  $('#dats').html(data);
                                                                                                                                                                           });
                                                       });

                                                       $('#str').change(function(){
                                                                                  $.post("/modelo/informes/cria/factmencli.php",{accion:'ldcli', str:$('#str').val()}, function(data){
                                                                                                                                                                                   $('#clis').html(data);
                                                                                                                                                                                   });
                                                                                  });
                                                       $("#str option[value=1]").attr("selected", "selected");
                                                       $('#str, #type').selectmenu({width: 350});
                                                       $.post("/modelo/informes/cria/factmencli.php",{accion:'ldcli', str:$('#str').val()}, function(data){$('#clis').html(data);});
    ...
    <br><br>
         <form id="upuda">
	           <fieldset class="ui-widget ui-widget-content ui-corner-all">
		                 <legend class="ui-widget ui-widget-header ui-corner-all">Facturacio Clientes</legend>
		                 <div id="mensaje"> </div>
		                 <fieldset class="ui-widget ui-widget-content ui-corner-all">
