...
$query = "select legajo, emple, fechav, tipo, dias
          ...
          where  (DATEDIFF(vto, date(now())) < 45) and (id in (6))
          order by vto asc, emple";
$result= ejecutarSQL($query) or die ($query);
if (mysql_num_rows($result) > 0){
$datas='<font color="#800000"><b><i><u>Libretas Trabajo a vencer los proximos 30 dias</u></i></b></font><BR><br>
   <table border="1" id="tablita">
      ...
      $color = '#C2D2D2';
           while ($data = mysql_fetch_array($result)){
                 $vto = $data['fechav'];
                 if ($data['dias'] < 11)
                    $rr = "<font color='#FF8080'>$data[fechav]</font>";
                 elseif($data['dias'] < 1){
                    $rr = "<font color='#FF0000'><b>$data[fechav]</b></font>";
                 }
                 $color = ($color == '#C2D2D2')?'#D0D0D0':'#C2D2D2';
                 $datas.="<tr bgcolor=\"$color\">
                            <td>$data[0]</td>
